...
library(MASS) # Tools for GLM fitting
...
#' The data includes aggregated values of hospital discharges,
#' relative risk of infection, population, precipitation, temperature,
#' aerosol concentration, and relative humidity.
...
#' and the response variable in the RN region using various methods:
...
#' - For "MI", mutual information is calculated from discretizations using mutinformation() from the infotheo package.
...
#' lag non-linear structures (DLNM), evaluating various exposure and lag
...
buscar_mejor_modelo <- function(data, variable, respuesta, max_lag, max_df_var, max_df_lag) {
  ...
  #' --------------------------------------------------------------------------
  #' Helper function to evaluate model fit metrics
  #' --------------------------------------------------------------------------
  ...
  #' --------------------------------------------------------------------------
  #' Initial base evaluation: linear function and strata lag structure
  #' --------------------------------------------------------------------------
  ...
  #' --------------------------------------------------------------------------
  #' Evaluation using cubic spline functions for both variable and lag
  #' --------------------------------------------------------------------------
  for (lag in 2:max_lag) {
...
max_lag <- 14
## 4.2 Correlational analysis in subregion RN1 ----
...
for (lag in 1:max_lag) {
  data_lag <- data_rn_subreg %>%
    filter(Subreg_climat == "RN1")
...
#' Evaluation of univariate DLNM models for temperature in subregion RN1

data_rn1 <- data_rn_subreg %>%
  filter(Subreg_climat == "RN1")
...
#' Compute correlations and p-values for each lag
for (lag in 1:max_lag) {
  data_rn1 <- data_rn_subreg %>% filter(Subreg_climat == "RN1")
  
  X_lagged <- head(data_rn1$precip_media, n = length(data_rn1$precip_media) - lag)
  Y_lagged <- tail(data_rn1$egreso_semana, n = length(data_rn1$egreso_semana) - lag)
...
#' Evaluation of univariate DLNM models for precipitation in subregion RN1

res <- buscar_mejor_modelo(
  data_rn_subreg %>% filter(Subreg_climat == "RN1"), # Filter subregion RN1
...
#' Display all evaluated models sorted by the composite score
...
for (lag in 1:max_lag) {
  df_filtered <- data_rn_subreg %>% filter(Subreg_climat == "RN1")
  ...
  #' Correlation metrics and p-values
  pearson_corr <- cor(X_lagged, Y_lagged)
  pearson_p <- perm_test(X_lagged, Y_lagged, method = "pearson")
...
#' Evaluation of univariate DLNM models for relative humidity in subregion RN1
...
#' Display all evaluated models sorted by composite score
...
for (lag in 1:max_lag) {
  datos <- data_rn_subreg %>% filter(Subreg_climat == "RN1")
...
#' Evaluation of univariate DLNM models for AOD in subregion RN1
res <- buscar_mejor_modelo(
  data_rn_subreg %>% filter(Subreg_climat == "RN1"), # Filter subregion RN1
...
## 4.3 Correlational analysis in subregion RN2 ----
...
#' Selection of the best DLNM model for mean temperature in subregion RN2

#' Filter RN2 climatic subregion and fit DLNM models
res <- buscar_mejor_modelo(
  data = data_rn_subreg %>% filter(Subreg_climat == "RN2"), # Filter subregion RN2
...
#' Evaluation of univariate DLNM models for precipitation in subregion RN2
res <- buscar_mejor_modelo(
  data = data_rn_subreg %>% filter(Subreg_climat == "RN2"), # Filter subregion RN2
  variable = "precip_media", # Explanatory variable
  respuesta = "egreso_semana", # Response variable
...
for (lag in 1:max_lag) {
  data_lag <- data_rn_subreg %>% filter(Subreg_climat == "RN2")
...
## 4.4 Correlational analysis in subregion RN3 ----
...
for (lag in 1:max_lag) {
  X_lagged <- head(
    (data_rn_subreg %>% filter(Subreg_climat == "RN3"))$Temp_media,
    length((data_rn_subreg %>% filter(Subreg_climat == "RN3"))$Temp_media) - lag
  )
  Y_lagged <- tail(
    (data_rn_subreg %>% filter(Subreg_climat == "RN3"))$egreso_semana,
    length((data_rn_subreg %>% filter(Subreg_climat == "RN3"))$egreso_semana) - lag
  )
...
#' Compute extrema for plot annotations
...
#' Filter RN3 climatic subregion

data_rn3 <- data_rn_subreg %>%
  filter(Subreg_climat == "RN3")
...
#' Fit DLNM models with various function and degree of freedom combinations

res <- buscar_mejor_modelo(
  data = data_rn_subreg %>% filter(Subreg_climat == "RN3"), # Filter subregion RN3
  variable = "RH", # Explanatory variable
  respuesta = "egreso_semana", # Response variable
...
## 4.5 Correlational analysis in subregion RN4 ----
...
for (lag in 1:max_lag) {
  X_lagged <- head(
    (data_rn_subreg %>% filter(Subreg_climat == "RN4"))$Temp_media,
    length((data_rn_subreg %>% filter(Subreg_climat == "RN4"))$Temp_media) - lag
  )
  Y_lagged <- tail(
    (data_rn_subreg %>% filter(Subreg_climat == "RN4"))$egreso_semana,
    length((data_rn_subreg %>% filter(Subreg_climat == "RN4"))$egreso_semana) - lag
  )
...
#' Filter RN4 climatic subregion

data_rn4 <- data_rn_subreg %>%
  filter(Subreg_climat == "RN4")
...
#' Compute extrema for each metric
...
res <- buscar_mejor_modelo(
  data = data_rn_subreg %>% filter(Subreg_climat == "RN4"), # Filter subregion RN4
  variable = "RH", # Explanatory variable
  respuesta = "egreso_semana", # Response variable
  max_lag = 7, # Maximum lag
  max_df_var = 3, # Maximum df for the variable
...
## 4.6 Correlational analysis in subregion RN5 ----
...
for (lag in 1:max_lag) {
  X_lagged <- head(
    (data_rn_subreg %>% filter(Subreg_climat == "RN5"))$Temp_media,
    length((data_rn_subreg %>% filter(Subreg_climat == "RN5"))$Temp_media) - lag
  )
  Y_lagged <- tail(
    (data_rn_subreg %>% filter(Subreg_climat == "RN5"))$egreso_semana,
    length((data_rn_subreg %>% filter(Subreg_climat == "RN5"))$egreso_semana) - lag
  )
...
#' Compute extreme values for annotations
...
#' Filter climatic subregion RN5

data_rn5 <- data_rn_subreg %>%
  filter(Subreg_climat == "RN5")

#' Search for the best model using the predefined function
...
res <- buscar_mejor_modelo(
  data = data_rn_subreg %>% filter(Subreg_climat == "RN5"), # Filter RN5 subregion
  variable = "RH", # Explanatory variable
  respuesta = "egreso_semana", # Response variable
  max_lag = 3, # Maximum lag
...
## 4.7 Combined Correlation Analysis Function ----
...
discrete_modeling_parameters <- function(
  ...
  # Define variables and their corresponding modeling parameters
  variables <- list(
  ...
  # Initialize data frame to store model results
  resultados <- data.frame(
    modelo = character(),
  ...
  # Internal function to evaluate the fitted model and compute performance metrics
  # Note: R2 term is excluded from the score formula.
  evaluar_modelo <- function(modelo, data_filtrada, respuesta, tipo, params) {
    ...
    # Adjust log-likelihood term for scoring function
  ...
  # Internal function to generate crossbasis objects using the specified lag parameters
  ...
  # Generate crossbasis objects for all predictors
  crossbases <- lapply(variables, generar_crossbasis, data = data)
  
  # Identify the largest lag to filter the dataset accordingly
  ...
  # Filter crossbasis matrices to align with the trimmed dataset
  ...
  # Nested loops to iterate over all combinations of basis function parameters
    ...
    for (cb_prec in crossbases_filtradas$prec) {
      for (cb_rh in crossbases_filtradas$rh) {
        for (cb_aod in crossbases_filtradas$aod) {
          ...
          # Fit the negative binomial regression model with distributed lag terms
...
#' This section defines the crossbasis matrices required for
#' DLNM-GLMM modeling of hospital discharges in subregions
#' RN1 through RN5, based on climatic and air pollution variables.

#' Sort dataset by climatic subregion
data_rn_subreg <- data_rn_subreg %>%
  arrange(Subreg_climat)

## 5.1 Crossbasis for Subregion RN1 ----

cb_t1 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN1") %>% pull(Temp_media),
                    lag = 6, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_p1 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN1") %>% pull(precip_media),
                    lag = 8, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_rh1 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN1") %>% pull(RH),
                     lag = 3, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_aod1 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN1") %>% pull(aerosol),
                      lag = 11, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)

## 5.3 Crossbasis for Subregion RN2 ----

cb_t2 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN2") %>% pull(Temp_media),
                    lag = 7, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_p2 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN2") %>% pull(precip_media),
                    lag = 10, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_rh2 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN2") %>% pull(RH),
                     lag = 3, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_aod2 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN2") %>% pull(aerosol),
                      lag = 9, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)

## 5.4 Crossbasis for Subregion RN3 ----

cb_t3 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN3") %>% pull(Temp_media),
                    lag = 6, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_p3 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN3") %>% pull(precip_media),
                    lag = 7, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_rh3 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN3") %>% pull(RH),
                     lag = 4, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_aod3 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN3") %>% pull(aerosol),
                      lag = 14, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)

## 5.5 Crossbasis for Subregion RN4 ----

cb_t4 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN4") %>% pull(Temp_media),
                    lag = 11, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_p4 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN4") %>% pull(precip_media),
                    lag = 7, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_rh4 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN4") %>% pull(RH),
                     lag = 7, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_aod4 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN4") %>% pull(aerosol),
                      lag = 11, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)

## 5.6 Crossbasis for Subregion RN5 ----

cb_t5 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN5") %>% pull(Temp_media),
                    lag = 7, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_p5 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN5") %>% pull(precip_media),
                    lag = 5, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_rh5 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN5") %>% pull(RH),
                     lag = 3, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_aod5 <- crossbasis(data_rn_subreg %>% filter(Subreg_climat == "RN5") %>% pull(aerosol),
                      lag = 13, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)

## 5.7 Combine Crossbasis Matrices with Data ----

#' Combine original data with all crossbasis matrices

df_con_crossbasis_egresos <- cbind(
  data_rn_subreg,
  setNames(as.data.frame(rbind(cb_t1, cb_t2, cb_t3, cb_t4, cb_t5)),
           make.names(paste0("T_", colnames(cb_t1)), unique = TRUE)),
  setNames(as.data.frame(rbind(cb_p1, cb_p2, cb_p3, cb_p4, cb_p5)),
           make.names(paste0("P_", colnames(cb_p1)), unique = TRUE)),
  setNames(as.data.frame(rbind(cb_rh1, cb_rh2, cb_rh3, cb_rh4, cb_rh5)),
           make.names(paste0("RH_", colnames(cb_rh1)), unique = TRUE)),
  setNames(as.data.frame(rbind(cb_aod1, cb_aod2, cb_aod3, cb_aod4, cb_aod5)),
           make.names(paste0("AOD_", colnames(cb_aod1)), unique = TRUE))
)

#' Remove initial rows per group based on the maximum lag (lag = 14)
...
#' Retrieve names of exposure columns from crossbasis matrices
...
## 5.8 Requirement for Zero-Inflated Models ----

#' Percentage of zero counts in all subregions
summary(data_rn_subreg$egreso_semana)
100*sum(data_rn_subreg$egreso_semana == 0)/(length(data_rn_subreg$egreso_semana))

#' Percentage of zero counts in each subregion
egreso_cero_rn1 <- data_rn_subreg %>% filter(Subreg_climat == "RN1") %>% pull(egreso_semana)
100*sum(egreso_cero_rn1 == 0, na.rm = TRUE)/(length(egreso_cero_rn1))

egreso_cero_rn2 <- data_rn_subreg %>% filter(Subreg_climat == "RN2") %>% pull(egreso_semana)
100*sum(egreso_cero_rn2 == 0, na.rm = TRUE)/(length(egreso_cero_rn2))

egreso_cero_rn3 <- data_rn_subreg %>% filter(Subreg_climat == "RN3") %>% pull(egreso_semana)
100*sum(egreso_cero_rn3 == 0, na.rm = TRUE)/(length(egreso_cero_rn3))

egreso_cero_rn4 <- data_rn_subreg %>% filter(Subreg_climat == "RN4") %>% pull(egreso_semana)
100*sum(egreso_cero_rn4 == 0, na.rm = TRUE)/(length(egreso_cero_rn4))

egreso_cero_rn5 <- data_rn_subreg %>% filter(Subreg_climat == "RN5") %>% pull(egreso_semana)
100*sum(egreso_cero_rn5 == 0, na.rm = TRUE)/(length(egreso_cero_rn5))
...
## 6.2 DLNM + GLMM Model (2): Including Weekly Seasonal Factors ----

modelo_dlnm_glmm_poi_rn_2 <- glmmTMB(
  egreso_semana ~
...
## 6.4 DLNM + GAMLSS Model (3): Including Weekly Seasonal Factors ----
...
## 7.2 DLNM + GLMM Model (2): Including Weekly Seasonal Factors ----

modelo_dlnm_glmm_nb_rn_2 <- glmmTMB(
  egreso_semana ~
    T_v1.l1 + T_v1.l2 + T_v1.l3 +
    T_v2.l1 + T_v2.l2 + T_v2.l3 +
    T_v3.l1 + T_v3.l2 + T_v3.l3 +
...
## 7.4 DLNM + GAMLSS Model (3): Including Weekly and Annual Seasonal Factors ----
...
#' This section applies the `calcular_metricas()` function to fitted models
#' (GLMM and GAMLSS, with various distributions), and builds a table
#' sorted by best model fit score.

#' Sort fitted values in dataset
df_con_crossbasis_egresos_rn <- df_con_crossbasis_egresos %>%
  mutate(
    #' GLMM Poisson Model
    ajustado_glmm_poi = fitted(modelo_glmm_poi_rn),
    
    #' DLNM + GLMM Poisson Model
    ajustado_dlnm_glmm_poi_2 = fitted(modelo_dlnm_glmm_poi_rn_2),
    
    #' GAMLSS Poisson Model
    ajustado_gamlss_poi_3 = fitted(modelo_gamlss_poi_3),
    
    #' GLMM Negative Binomial Model
    ajustado_glmm_nb = fitted(modelo_glmm_nb_rn),
    
    #' DLNM + GLMM Negative Binomial Model
    ajustado_dlnm_glmm_nb_2 = fitted(modelo_dlnm_glmm_nb_rn_2),
...
#' ------------------------------------------------------------------------------
#' Computes performance metrics for a fitted model and its associated predictions.
#' Includes standard error metrics, information criteria, and a composite score
...
calcular_metricas <- function(modelo, predicciones, obs, nombre_modelo) {
  ...
  # MAPE with epsilon correction to prevent division by zero
  epsilon <- 1e-10
  ...
  # Composite score: inverse errors + adjusted log-likelihood
...
resultados_modelos <- rbind(
  calcular_metricas(
    modelo_glmm_poi_rn,
    df_con_crossbasis_egresos_rn$ajustado_glmm_poi,
    df_con_crossbasis_egresos_rn$egreso_semana,
    "GLMM_Poi"
  ),
  calcular_metricas(
    modelo_dlnm_glmm_poi_rn_2,
    df_con_crossbasis_egresos_rn$ajustado_dlnm_glmm_poi_2,
    df_con_crossbasis_egresos_rn$egreso_semana,
    "DLNM_GLMM_Poi_2"
  ),
  calcular_metricas(
    modelo_gamlss_poi_3,
    df_con_crossbasis_egresos_rn$ajustado_gamlss_poi_3,
    df_con_crossbasis_egresos_rn$egreso_semana,
    "GAMLSS_Poi_3"
  ),
  calcular_metricas(
    modelo_glmm_nb_rn,
    df_con_crossbasis_egresos_rn$ajustado_glmm_nb,
    df_con_crossbasis_egresos_rn$egreso_semana,
    "GLMM_NB"
  ),
  calcular_metricas(
    modelo_dlnm_glmm_nb_rn_2,
    df_con_crossbasis_egresos_rn$ajustado_dlnm_glmm_nb_2,
    df_con_crossbasis_egresos_rn$egreso_semana,
    "DLNM_GLMM_NB_2"
  ),
  calcular_metricas(
    modelo_gamlss_nb_3,
    df_con_crossbasis_egresos_rn$ajustado_gamlss_nb_3,
    df_con_crossbasis_egresos_rn$egreso_semana,
    "GAMLSS_NB_3"
  )
)

# Rank models from best to worst based on the composite score
...
dlnm_glmm_model_rn <- modelo_gamlss_poi_3
summary(dlnm_glmm_model_rn)
## 8.3 Metrics for Filtered Subregions ----

#' Time series construction by model for RN1
df_rn1 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN1") %>%
  mutate(
...
# Time series construction for RN2
df_rn2 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN2") %>%
  mutate(
    egresos_ts = ts(egreso_semana, start = c(2000, 15), frequency = 52),
    glmm_ts = ts(ajustado_glmm_nb, start = c(2000, 15), frequency = 52),
...
# Time series construction for RN3
df_rn3 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN3") %>%
  mutate(
    egresos_ts = ts(egreso_semana, start = c(2000, 15), frequency = 52),
    glmm_ts = ts(ajustado_glmm_nb, start = c(2000, 15), frequency = 52),
...
# Time series construction for RN4
df_rn4 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN4") %>%
  mutate(
    egresos_ts = ts(egreso_semana, start = c(2000, 15), frequency = 52),
    glmm_ts = ts(ajustado_glmm_nb, start = c(2000, 15), frequency = 52),
...
# Time series construction for RN5
df_rn5 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN5") %>%
  mutate(
    egresos_ts = ts(egreso_semana, start = c(2000, 15), frequency = 52),
    glmm_ts = ts(ajustado_glmm_nb, start = c(2000, 15), frequency = 52),
...
calcular_metricas_sub <- function(df, subregion, col_obs = "egresos_ts") {
  ...
  #' Returns the two best predictions based on lowest composite Score
  ...
  results <- lapply(model_columns, function(model_name) {
    pred <- df[[model_name]]
  ...
  return(head(results, 2))
}

# Apply the function by subregion
resultados_modelos <- rbind(
  calcular_metricas_sub(df_rn1, "RN1", "egreso_semana"),
  calcular_metricas_sub(df_rn2, "RN2", "egreso_semana"),
  calcular_metricas_sub(df_rn3, "RN3", "egreso_semana"),
  calcular_metricas_sub(df_rn4, "RN4", "egreso_semana"),
  calcular_metricas_sub(df_rn5, "RN5", "egreso_semana")
)
...
resid_simple_rn <- residuals(dlnm_glmm_model_rn, type = "simple")

#' Weighted residuals (similar to Pearson residuals)
resid_weighted_rn <- residuals(dlnm_glmm_model_rn, type = "weighted")

#' Partial residuals (used to evaluate partial effects)
resid_partial_rn <- residuals(dlnm_glmm_model_rn, type = "partial")

#' Dataset including model residuals
df_con_crossbasis_egresos_rn <- df_con_crossbasis_egresos_rn %>%
  mutate(
    resid_simple_rn = resid_simple_rn,
    resid_weighted_rn = resid_weighted_rn,
    resid_partial_rn = resid_partial_rn
  )
...
qqnorm(resid_simple_rn, main = "Simple residuals")
qqline(resid_simple_rn, col = "red")

qqnorm(resid_weighted_rn, main = "Weighted residuals")
qqline(resid_weighted_rn, col = "red")

# Perform Shapiro-Wilk normality test
shapiro_test_simple <- shapiro.test(resid_simple_rn)
shapiro_test_weighted <- shapiro.test(resid_weighted_rn)
...
resid_simple_rn1 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN1") %>%
  pull(resid_simple_rn)

resid_simple_rn2 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN2") %>%
  pull(resid_simple_rn)

resid_simple_rn3 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN3") %>%
  pull(resid_simple_rn)

resid_simple_rn4 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN4") %>%
  pull(resid_simple_rn)

resid_simple_rn5 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN5") %>%
  pull(resid_simple_rn)

#' Extract weighted residuals by climatic subregion

resid_weighted_rn1 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN1") %>%
  pull(resid_weighted_rn)

resid_weighted_rn2 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN2") %>%
  pull(resid_weighted_rn)

resid_weighted_rn3 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN3") %>%
  pull(resid_weighted_rn)

resid_weighted_rn4 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN4") %>%
  pull(resid_weighted_rn)

resid_weighted_rn5 <- df_con_crossbasis_egresos_rn %>%
  filter(Subreg_climat == "RN5") %>%
  pull(resid_weighted_rn)

#' Time series diagnostic plots for simple residuals by subregion
ggtsdisplay(resid_simple_rn1) # Subregion RN1
ggtsdisplay(resid_simple_rn2) # Subregion RN2
ggtsdisplay(resid_simple_rn3) # Subregion RN3
ggtsdisplay(resid_simple_rn4) # Subregion RN4
ggtsdisplay(resid_simple_rn5) # Subregion RN5

#' Time series diagnostic plots for weighted residuals by subregion
ggtsdisplay(resid_weighted_rn1) # Subregion RN1
ggtsdisplay(resid_weighted_rn2) # Subregion RN2
ggtsdisplay(resid_weighted_rn3) # Subregion RN3
ggtsdisplay(resid_weighted_rn4) # Subregion RN4
ggtsdisplay(resid_weighted_rn5) # Subregion RN5
# 10. Adjusted Effects from the Model ----

## 10.1 Comparative Adjustment Plots ----

#' Comparative plot of predicted values – Subregion RN1
#' Comparison plot: observed vs. GLMM and GAMLSS models
autoplot(df_rn1$egresos_ts, series = "Observed") +
  autolayer(df_rn1$glmm_ts, series = "GLMM NB") +
  autolayer(df_rn1$gamlss_ts, series = "DLNM - GAMLSS POI") +
  labs(
    x = "Time (weeks)",
...
#' Comparative plot of predicted values – Subregion RN2
autoplot(df_rn2$egresos_ts, series = "Observed") +
  autolayer(df_rn2$glmm_ts, series = "GLMM NB") +
  autolayer(df_rn2$gamlss_ts, series = "DLNM - GAMLSS") +
  labs(
    x = "Time (weeks)",
    y = "Hospital admissions"
  ) +
...
#' Comparative plot of predicted values – Subregion RN3
autoplot(df_rn3$egresos_ts, series = "Observed") +
  autolayer(df_rn3$glmm_ts, series = "GLMM NB") +
  autolayer(df_rn3$gamlss_ts, series = "DLNM - GAMLSS") +
  labs(
    x = "Time (weeks)",
    y = "Hospital admissions"
  ) +
...
#' Comparative plot of predicted values – Subregion RN4
autoplot(df_rn4$egresos_ts, series = "Observed") +
  autolayer(df_rn4$glmm_ts, series = "GLMM NB") +
  autolayer(df_rn4$gamlss_ts, series = "DLNM - GAMLSS") +
  labs(
    x = "Time (weeks)",
    y = "Hospital admissions"
  ) +
...
#' Comparative plot of predicted values – Subregion RN5
autoplot(df_rn5$egresos_ts, series = "Observed") +
  autolayer(df_rn5$glmm_ts, series = "GLMM NB") +
  autolayer(df_rn5$gamlss_ts, series = "DLNM - GAMLSS") +
  labs(
    x = "Time (weeks)",
    y = "Hospital admissions"
  ) +
...
## 10.2 Predictions ----
#' An example is presented for a specific subregion.

#' Extract fixed coefficients and full covariance matrix from the DLNM-GAMLSS Poisson model
coef_model <- coef(modelo_gamlss_poi_3)

vcov_model <- vcov(modelo_gamlss_poi_3, full = TRUE)
...
# Save the models in the designated folder
save(
  modelo_glmm_poi_rn,
  modelo_dlnm_glmm_poi_rn_2,
  modelo_gamlss_poi_3,
  ...
  coef_model,
  vcov_model,
  file = "random_effects_models/dlnm_models_for_discharges_rn.RData"
)

# Load the saved models
#load("random_effects_models/dlnm_models_for_discharges.RData")

#' Extract coefficients and var-cov matrix for temperature (cb_t1 - subregion RN1)
coef_t <- coef_model[grep("^T_", names(coef_model))]

vcov_t <- as.matrix(
  vcov_model[
    grep("^T_", names(coef_model)),
    grep("^T_", names(coef_model))
  ]
)

#' Extract coefficients and var-cov matrix for precipitation (cb_p1 - subregion RN1)
coef_p <- coef_model[grep("^P_", names(coef_model))]

vcov_p <- as.matrix(
  vcov_model[
    grep("^P_", names(coef_model)),
    grep("^P_", names(coef_model))
  ]
)

#' Extract coefficients and var-cov matrix for relative humidity (cb_rh1 - subregion RN1)
coef_rh <- coef_model[grep("^RH_", names(coef_model))]

vcov_rh <- as.matrix(
  vcov_model[
    grep("^RH_", names(coef_model)),
    grep("^RH_", names(coef_model))
  ]
)

#' Extract coefficients and var-cov matrix for aerosols (cb_aod1 - subregion RN1)
coef_aod <- coef_model[grep("^AOD_", names(coef_model))]

vcov_aod <- as.matrix(
  vcov_model[
    grep("^AOD_", names(coef_model)),
    grep("^AOD_", names(coef_model))
  ]
)

#' Generate predictions for temperature using crosspred (DLNM-GLMM, subregion RN1)
pred_dlnm_glmm_rn1_temp <- crosspred(
  cb_t1,
  coef = coef_t,
  vcov = vcov_t,
  model.link = "log",
...
#' Generate predictions for precipitation using crosspred (DLNM-GLMM, subregion RN1)
pred_dlnm_glmm_rn1_prec <- crosspred(
  cb_p1,
  coef = coef_p,
  vcov = vcov_p,
  model.link = "log",
...
#' Generate predictions for relative humidity using crosspred (DLNM-GLMM, subregion RN1)
pred_dlnm_glmm_rn1_rh <- crosspred(
  cb_rh1,
  coef = coef_rh,
  vcov = vcov_rh,
  model.link = "log",
...
#' Generate predictions for AOD using crosspred (DLNM-GLMM, subregion RN1)
pred_dlnm_glmm_rn1_aod <- crosspred(
  cb_aod1,
  coef = coef_aod,
  vcov = vcov_aod,
  model.link = "log",
...
#' Contour plot of the lag-response relationship for mean temperature
plot(
  pred_dlnm_glmm_rn1_temp,
  "contour", # valid first argument
  xlab = "Mean temperature",
  ylab = "Lag",
  key.title = title("RR")
)

#' Lag-response plot for a specific value of temperature (var = 30.3 °C)
plot(
  pred_dlnm_glmm_rn1_temp,
  "slices", # unnamed second argument
  var = max(pred_dlnm_glmm_rn1_temp$predvar), # specific predictor value
  col = 2,
  ylab = "RR"
)

#' Lag-response plot for another specific temperature value (var = 23.5 °C)
plot(
  pred_dlnm_glmm_rn1_temp,
  "slices",
  var = min(pred_dlnm_glmm_rn1_temp$predvar),
  col = 2,
  ylab = "RR"
)
...
#' Contour plot of the lag-response relationship for mean precipitation
plot(
  pred_dlnm_glmm_rn1_prec,
  "contour",
  xlab = "Mean precipitation",
...
#' Contour plot of the lag-response relationship for relative humidity
plot(
  pred_dlnm_glmm_rn1_rh,
  "contour",
  xlab = "Relative humidity",
...
#' Lag-response plot for another specific value of relative humidity (var = 0.45)
plot(
  pred_dlnm_glmm_rn1_rh,
  "slices",
  var = max(pred_dlnm_glmm_rn1_rh$predvar),
  col = 2,
  ylab = "RR"
)
...
#' Contour plot of the lag-response relationship for AOD
plot(
  pred_dlnm_glmm_rn1_aod,
  "contour",
  xlab = "Mean AOD index",
...
#' Generate predictions for temperature using crosspred (DLNM-GLMM, subregion RN2)
pred_dlnm_glmm_rn2_temp <- crosspred(
  cb_t2,
  coef = coef_t,
  vcov = vcov_t,
  model.link = "log",
...
#' Generate predictions for precipitation using crosspred (DLNM-GLMM, subregion RN2)
pred_dlnm_glmm_rn2_prec <- crosspred(
  cb_p2,
  coef = coef_p,
  vcov = vcov_p,
  model.link = "log",
...
#' Generate predictions for relative humidity using crosspred (DLNM-GLMM, subregion RN2)
pred_dlnm_glmm_rn2_rh <- crosspred(
  cb_rh2,
  coef = coef_rh,
  vcov = vcov_rh,
  model.link = "log",
...
#' Generate predictions for AOD using crosspred (DLNM-GLMM, subregion RN2)
pred_dlnm_glmm_rn2_aod <- crosspred(
  cb_aod2,
  coef = coef_aod,
  vcov = vcov_aod,
  model.link = "log",
...
#' Generate predictions for temperature using crosspred (DLNM-GLMM, subregion RN3)
pred_dlnm_glmm_rn3_temp <- crosspred(
  cb_t3,
  coef = coef_t,
  vcov = vcov_t,
  model.link = "log",
...
#' Generate predictions for precipitation using crosspred (DLNM-GLMM, subregion RN3)
pred_dlnm_glmm_rn3_prec <- crosspred(
  cb_p3,
  coef = coef_p,
  vcov = vcov_p,
  model.link = "log",
...
#' Generate predictions for relative humidity using crosspred (DLNM-GLMM, subregion RN3)
pred_dlnm_glmm_rn3_rh <- crosspred(
  cb_rh3,
  coef = coef_rh,
  vcov = vcov_rh,
  model.link = "log",
...
#' Generate predictions for AOD using crosspred (DLNM-GLMM, subregion RN3)
pred_dlnm_glmm_rn3_aod <- crosspred(
  cb_aod3,
  coef = coef_aod,
  vcov = vcov_aod,
  model.link = "log",
...
#' Generate predictions for temperature using crosspred (DLNM-GLMM, subregion RN4)
pred_dlnm_glmm_rn4_temp <- crosspred(
  cb_t3,
  coef = coef_t,
  vcov = vcov_t,
  model.link = "log",
...
#' Generate predictions for precipitation using crosspred (DLNM-GLMM, subregion RN4)
pred_dlnm_glmm_rn4_prec <- crosspred(
  cb_p3,
  coef = coef_p,
  vcov = vcov_p,
  model.link = "log",
...
#' Generate predictions for relative humidity using crosspred (DLNM-GLMM, subregion RN4)
pred_dlnm_glmm_rn4_rh <- crosspred(
  cb_rh3,
  coef = coef_rh,
  vcov = vcov_rh,
  model.link = "log",
...
#' Generate predictions for AOD using crosspred (DLNM-GLMM, subregion RN4)
pred_dlnm_glmm_rn4_aod <- crosspred(
  cb_aod3,
  coef = coef_aod,
  vcov = vcov_aod,
  model.link = "log",
...
#' Generate predictions for temperature using crosspred (DLNM-GLMM, subregion RN5)
pred_dlnm_glmm_rn5_temp <- crosspred(
  cb_t3,
  coef = coef_t,
  vcov = vcov_t,
  model.link = "log",
...
#' Generate predictions for precipitation using crosspred (DLNM-GLMM, subregion RN5)
pred_dlnm_glmm_rn5_prec <- crosspred(
  cb_p3,
  coef = coef_p,
  vcov = vcov_p,
  model.link = "log",
...
#' Generate predictions for relative humidity using crosspred (DLNM-GLMM, subregion RN5)
pred_dlnm_glmm_rn5_rh <- crosspred(
  cb_rh3,
  coef = coef_rh,
  vcov = vcov_rh,
  model.link = "log",
...
#' Generate predictions for AOD using crosspred (DLNM-GLMM, subregion RN5)
pred_dlnm_glmm_rn5_aod <- crosspred(
  cb_aod3,
  coef = coef_aod,
  vcov = vcov_aod,
  model.link = "log",
...
# set common levels for year in both train and test
...
# full formula for GAMLSS with random effects by subregion
...
df_train <- df_train %>%
  mutate(ajust_gamlss = predict(modelo_gamlss_poi, type = "response"))

df_test <- df_test %>%
  mutate(pred_gamlss = predict(modelo_gamlss_poi, newdata = df_test, type = "response"))
