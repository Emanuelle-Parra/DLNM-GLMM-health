...
# The data includes summary values of discharges, relative risk of contagion,
...
## 3.2 Data aggregated by epidemiological week for each region ----
...
#' This block designs a correlation function between lagged predictor variables
#' and the response variable in the RN region using different methods:
...
## 4.1 Functions to establish crossbasis parametrization ----
...
#' - For "MI", mutual information is computed between discretizations using mutinformation() from infotheo.
...
#' non-linear lag structures (DLNM), evaluating various types of
...
## 4.2 Correlation analysis in subregion RN1 ----
...
for (lag in 1:max_lag) {
  ...
  Y_lagged <- tail(data_rn1$Riesgo_relativ_contagio,
                   n = length(data_rn1$Riesgo_relativ_contagio) - lag)
...
res <- buscar_mejor_modelo(
  data_rn_subreg %>% filter(Subreg_climat == "RN1"), # Filter for subregion RN1
...
#' Display all evaluated models ordered by combined score
...
for (lag in 1:max_lag) {
  df_filtrada <- data_rn_subreg %>% filter(Subreg_climat == "RN1")
...
res <- buscar_mejor_modelo(
  ...
  variable = "RH", # predictor variable
  respuesta = "Riesgo_relativ_contagio", # response variable
...
#' Plot the correlation coefficients for each method
...
res <- buscar_mejor_modelo(
  data_rn_subreg %>% filter(Subreg_climat == "RN1"), # Filter subregion RN1
  variable = "aerosol", # Predictor variable (AOD)
  respuesta = "Riesgo_relativ_contagio", # Response variable
  max_lag = 11, # Maximum lag
  max_df_var = 3, # Maximum df for the variable
  max_df_lag = 3 # Maximum df for lags
...
## 4.3 Correlation analysis in subregion RN2 ----
...
#' Selection of best DLNM model for mean temperature in subregion RN2
res <- buscar_mejor_modelo(
  data = data_rn_subreg %>% filter(Subreg_climat == "RN2"), # Filter subregion RN2
  variable = "Temp_media", # Predictor variable
  respuesta = "Riesgo_relativ_contagio", # Response variable
...
res <- buscar_mejor_modelo(
  ...
  #' Compute maxima and minima for annotation in the plot
  ...
  res <- buscar_mejor_modelo(
    data = data_rn_subreg %>% filter(Subreg_climat == "RN2"), # Filter subregion RN2
    variable = "aerosol", # Predictor variable
    respuesta = "Riesgo_relativ_contagio", # Response variable
    max_lag = 11, # Maximum lag
    max_df_var = 3, # Maximum df for the variable
    max_df_lag = 3 # Maximum df for the lags
  ...
  ## 4.4 Correlation analysis in subregion RN3 ----
  ...
  for (lag in 1:max_lag) {
    ...
    Y_lagged <- tail(
      (data_rn_subreg %>% filter(Subreg_climat == "RN3"))$Riesgo_relativ_contagio,
      length((data_rn_subreg %>% filter(Subreg_climat == "RN3"))$Riesgo_relativ_contagio) - lag
    )
  ...
  #' Compute extrema for annotation in plot
  ...
  #' Identify best DLNM model using previously defined function
  ...
  ## 4.5 Correlation analysis in subregion RN4 ----
  ...
  for (lag in 1:max_lag) {
    ...
    Y_lagged <- tail(
      (data_rn_subreg %>% filter(Subreg_climat == "RN4"))$Riesgo_relativ_contagio,
      length((data_rn_subreg %>% filter(Subreg_climat == "RN4"))$Riesgo_relativ_contagio) - lag
    )
  ...
  #' Compute extrema for annotation in the plot
  ...
  ## 4.6 Correlational analysis in the RN5 subregion ----
  ...
  for (lag in 1:max_lag) {
    ...
    Y_lagged <- tail(
      (data_rn_subreg %>% filter(Subreg_climat == "RN5"))$Riesgo_relativ_contagio,
      length((data_rn_subreg %>% filter(Subreg_climat == "RN5"))$Riesgo_relativ_contagio) - lag
    )
  ...
  #' Compute min and max values for annotation purposes
  ...
  #' Plot correlation metrics by lag
  ...
  #' Filter the RN5 climatic subregion
  
  data_rn5 <- data_rn_subreg %>%
    filter(Subreg_climat == "RN5")
  
  #' Identify the best model using the pre-defined function
...
#' Visualization of correlation metrics by lag
...
#' distributed lag models. This function was implemented at the subregional 
#' level using all exposure variables to adequately parameterize the non-linear 
#' base functions.
#' 
#' Inputs:
...
continuous_modeling_parameters <- function(
  ...
  # Function to generate crossbasis objects using the specified maximum lag
  ...
  # Generate crossbasis objects for each variable
  crossbases <- lapply(variables, generar_crossbasis, data = data)
  
  # Determine the maximum lag across all crossbasis to filter the dataset
    ...
    for (cb_prec in crossbases_filtradas$prec) {
      for (cb_rh in crossbases_filtradas$rh) {
        for (cb_aod in crossbases_filtradas$aod) {
          ...
          # MODEL (other continuous functions could be used for validation;
          # parameter behavior remains consistent)
...
#' This section defines the crossbasis functions required for DLNM-GLMM modeling 
#' of hospitalization risks in subregions RN1, RN2, and RN3, based on climatic 
...
## 5.7 Merging Crossbasis Matrices -----
...
## Need for Zero-Inflation in Gamma Models ----

#' Percentage of zeros in all subregions

summary(data_rn_subreg$Riesgo_relativ_contagio)

100 * sum(data_rn_subreg$Riesgo_relativ_contagio == 0) /
  length(data_rn_subreg$Riesgo_relativ_contagio)

#' Percentage of zeros in subregion RN1

riesgo_rn1 <- data_rn_subreg %>%
  filter(Subreg_climat == "RN1") %>%
  pull(Riesgo_relativ_contagio)

100 * sum(riesgo_rn1 == 0, na.rm = TRUE) / length(riesgo_rn1)

#' Percentage of zeros in subregion RN2

riesgo_rn2 <- data_rn_subreg %>%
  filter(Subreg_climat == "RN2") %>%
  pull(Riesgo_relativ_contagio)

100 * sum(riesgo_rn2 == 0, na.rm = TRUE) / length(riesgo_rn2)

#' Percentage of zeros in subregion RN3

riesgo_rn3 <- data_rn_subreg %>%
  filter(Subreg_climat == "RN3") %>%
  pull(Riesgo_relativ_contagio)

100 * sum(riesgo_rn3 == 0, na.rm = TRUE) / length(riesgo_rn3)

#' Percentage of zeros in subregion RN4

riesgo_rn4 <- data_rn_subreg %>%
  filter(Subreg_climat == "RN4") %>%
  pull(Riesgo_relativ_contagio)

100 * sum(riesgo_rn4 == 0, na.rm = TRUE) / length(riesgo_rn4)

#' Percentage of zeros in subregion RN5

riesgo_rn5 <- data_rn_subreg %>%
  filter(Subreg_climat == "RN5") %>%
  pull(Riesgo_relativ_contagio)

100 * sum(riesgo_rn5 == 0, na.rm = TRUE) / length(riesgo_rn5)

# 6. DLNM-GLMM and DLNM-GAMLSS Models ----

## 6.1 GLMM Normal Model (1): simple specification ----

modelo_glmm_normal_rn <- glmmTMB(
...
modelo_dlnm_glmm_normal_rn <- glmmTMB(
...
modelo_dlnm_gamlss_inv_gauss_rn <- gamlss(
  Riesgo_relativ_contagio ~ T_v1.l1 + T_v1.l2 + T_v1.l3 + 
    T_v2.l1 + T_v2.l2 + T_v2.l3 + T_v3.l1 + T_v3.l2 + T_v3.l3 + 
    P_v1.l1 + P_v1.l2 + P_v1.l3 + P_v2.l1 + P_v2.l2 + P_v2.l3 + 
    P_v3.l1 + P_v3.l2 + P_v3.l3 + RH_v1.l1 + RH_v1.l2 + RH_v1.l3 + 
...
modelo_dlnm_gamlss_gamma_rn <- gamlss(
  Riesgo_relativ_contagio ~ T_v1.l1 + T_v1.l2 + T_v1.l3 + 
    T_v2.l1 + T_v2.l2 + T_v2.l3 + T_v3.l1 + T_v3.l2 + T_v3.l3 + 
    P_v1.l1 + P_v1.l2 + P_v1.l3 + P_v2.l1 + P_v2.l2 + P_v2.l3 + 
    P_v3.l1 + P_v3.l2 + P_v3.l3 + RH_v1.l1 + RH_v1.l2 + RH_v1.l3 + 
...
#' by the best model performance score.

#' Sort fitted values into the dataset

df_con_crossbasis_riesgos_rn <- df_con_crossbasis_riesgos %>%
  mutate(
    #' GLMM Normal Model
    ajustado_modelo_glmm_normal_rn = fitted(modelo_glmm_normal_rn),
    
    #' DLNM + GLMM Normal Model
    ajustado_modelo_dlnm_glmm_normal_rn = fitted(modelo_dlnm_glmm_normal_rn),
    
    #' DLNM + GAMLSS Models
    ajustado_modelo_dlnm_gamlss_inv_gauss_rn = fitted(modelo_dlnm_gamlss_inv_gauss_rn),
    ajustado_modelo_dlnm_gamlss_gamma_rn = fitted(modelo_dlnm_gamlss_gamma_rn)
  )
...
#' and a composite score summarizing overall model performance.
#'
#' Parameters:
...
calcular_metricas <- function(modelo, predicciones, obs, nombre_modelo) {
  ...
  ernilon <- 1e-10
  MAPE_val <- mean(abs((obs - predicciones) / (obs + ernilon))) * 100
  ...
  #' --------------------------------------------------------------------------
  #' Composite score: inverses + adjusted log-likelihood
  #' --------------------------------------------------------------------------
...
# Apply metric calculations to all models

resultados_modelos <- rbind(
  calcular_metricas(
    modelo_glmm_normal_rn,
    df_con_crossbasis_riesgos_rn$ajustado_modelo_glmm_normal_rn,
    df_con_crossbasis_riesgos_rn$Riesgo_relativ_contagio,
    "GLMM_Normal"
  ),
  calcular_metricas(
    modelo_dlnm_glmm_normal_rn,
    df_con_crossbasis_riesgos_rn$ajustado_modelo_dlnm_glmm_normal_rn,
    df_con_crossbasis_riesgos_rn$Riesgo_relativ_contagio,
    "DLNM_GLMM_Normal"
  ),
  calcular_metricas(
    modelo_dlnm_gamlss_inv_gauss_rn,
    df_con_crossbasis_riesgos_rn$ajustado_modelo_dlnm_gamlss_inv_gauss_rn,
    df_con_crossbasis_riesgos_rn$Riesgo_relativ_contagio,
    "DLNM_GAMLSS_INV_GAUSS"
  ),
  calcular_metricas(
    modelo_dlnm_gamlss_gamma_rn,
    df_con_crossbasis_riesgos_rn$ajustado_modelo_dlnm_gamlss_gamma_rn,
    df_con_crossbasis_riesgos_rn$Riesgo_relativ_contagio,
    "DLNM_GAMLSS_GAMMA"
  )
)

#' Sort models from best to worst based on the composite score
...
dlnm_glmm_model_rn <- modelo_dlnm_glmm_normal_rn
summary(dlnm_glmm_model_rn)

## 7.3 Metrics for Filtered Subregions ----

#' Construct time series by model for RN1

df_rn1 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN1") %>%
  mutate(
    riesgos_ts = ts(Riesgo_relativ_contagio, start = c(2000, 15), frequency = 52),
    glmm_ts = ts(ajustado_modelo_glmm_normal_rn, start = c(2000, 15), frequency = 52),
    glm_dlnm_ts = ts(ajustado_modelo_dlnm_glmm_normal_rn, start = c(2000, 15), frequency = 52)
  )

#' Construct time series by model for RN2

df_rn2 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN2") %>%
  mutate(
    riesgos_ts = ts(Riesgo_relativ_contagio, start = c(2000, 15), frequency = 52),
    glmm_ts = ts(ajustado_modelo_glmm_normal_rn, start = c(2000, 15), frequency = 52),
...
#' Construct time series by model for RN3

df_rn3 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN3") %>%
  mutate(
    riesgos_ts = ts(Riesgo_relativ_contagio, start = c(2000, 15), frequency = 52),
    glmm_ts = ts(ajustado_modelo_glmm_normal_rn, start = c(2000, 15), frequency = 52),
...
#' Construct time series by model for RN4

df_rn4 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN4") %>%
  mutate(
    riesgos_ts = ts(Riesgo_relativ_contagio, start = c(2000, 15), frequency = 52),
    glmm_ts = ts(ajustado_modelo_glmm_normal_rn, start = c(2000, 15), frequency = 52),
...
#' Construct time series by model for RN5

df_rn5 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN5") %>%
  mutate(
    riesgos_ts = ts(Riesgo_relativ_contagio, start = c(2000, 15), frequency = 52),
    glmm_ts = ts(ajustado_modelo_glmm_normal_rn, start = c(2000, 15), frequency = 52),
...
#' Function to compute error metrics by subregion

calcular_metricas_sub <- function(df, subregion, col_obs = "riesgos_ts") {
  #' --------------------------------------------------------------------------
  #' Computes error metrics for fitted models by subregion
  #' Returns the two best predictions according to lowest score
  #' --------------------------------------------------------------------------
  
  #' Identify prediction columns (ending in _ts), excluding the observed series
  ...
  resultados <- lapply(columnas_modelo, function(nombre_modelo) {
    ...
    MSE <- mean((obs - pred)^2)
    RMSE <- sqrt(MSE)
    MAPE_val <- mean(abs((obs - pred) / (obs + ernilon))) * 100
    
    #' Composite score as the sum of the inverses of error metrics
...
resultados_modelos <- rbind(
  calcular_metricas_sub(df_rn1, "RN1", "Riesgo_relativ_contagio"),
  calcular_metricas_sub(df_rn2, "RN2", "Riesgo_relativ_contagio"),
  calcular_metricas_sub(df_rn3, "RN3", "Riesgo_relativ_contagio"),
  calcular_metricas_sub(df_rn4, "RN4", "Riesgo_relativ_contagio"),
  calcular_metricas_sub(df_rn5, "RN5", "Riesgo_relativ_contagio")
)

#' Sort models from best to worst according to the computed score
...
df_con_crossbasis_riesgos_rn <- df_con_crossbasis_riesgos_rn %>%
  mutate(
    resid_pearson_rn = resid_pearson_rn,
    resid_deviance_rn = resid_deviance_rn
  )
...
qqnorm(resid_pearson_rn, main = "Pearson Residuals")
qqline(resid_pearson_rn, col = "red")

qqnorm(resid_deviance_rn, main = "Deviance Residuals")
...
resid_pearson_rn1 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN1") %>%
  pull(resid_pearson_rn)

resid_pearson_rn2 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN2") %>%
  pull(resid_pearson_rn)

resid_pearson_rn3 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN3") %>%
  pull(resid_pearson_rn)

resid_pearson_rn4 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN4") %>%
  pull(resid_pearson_rn)

resid_pearson_rn5 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN5") %>%
  pull(resid_pearson_rn)

#' Extract deviance residuals by climatic subregion

resid_deviance_rn1 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN1") %>%
  pull(resid_deviance_rn)

resid_deviance_rn2 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN2") %>%
  pull(resid_deviance_rn)

resid_deviance_rn3 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN3") %>%
  pull(resid_deviance_rn)

resid_deviance_rn4 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN4") %>%
  pull(resid_deviance_rn)

resid_deviance_rn5 <- df_con_crossbasis_riesgos_rn %>%
  filter(Subreg_climat == "RN5") %>%
  pull(resid_deviance_rn)

#' Time series diagnostic plots for Pearson residuals by subregion

ggtsdisplay(resid_pearson_rn1) # Subregion RN1
ggtsdisplay(resid_pearson_rn2) # Subregion RN2
ggtsdisplay(resid_pearson_rn3) # Subregion RN3
ggtsdisplay(resid_pearson_rn4) # Subregion RN4
ggtsdisplay(resid_pearson_rn5) # Subregion RN5

#' Time series diagnostic plots for deviance residuals by subregion

ggtsdisplay(resid_deviance_rn1) # Subregion RN1
ggtsdisplay(resid_deviance_rn2) # Subregion RN2
ggtsdisplay(resid_deviance_rn3) # Subregion RN3
ggtsdisplay(resid_deviance_rn4) # Subregion RN4
ggtsdisplay(resid_deviance_rn5) # Subregion RN5
# 9. Fitted Effects of the Model ----

## 9.1 Comparative Plots of Model Fits ----

#' Comparative plot of predicted values - Subregion RN1
autoplot(df_rn1$riesgos_ts, series = "Observed") +
  autolayer(df_rn1$glmm_ts, series = "GLMM") +
  autolayer(df_rn1$glm_dlnm_ts, series = "DLNM - GLMM") +
  labs(
    x = "Time (weeks)",
...
autoplot(df_rn2$riesgos_ts, series = "Observed") +
  autolayer(df_rn2$glmm_ts, series = "GLMM") +
  autolayer(df_rn2$glm_dlnm_ts, series = "DLNM - GLMM") +
  labs(
    x = "Time (weeks)",
    y = "Hospital discharge risk"
  ) +
...
autoplot(df_rn3$riesgos_ts, series = "Observed") +
  autolayer(df_rn3$glmm_ts, series = "GLMM") +
  autolayer(df_rn3$glm_dlnm_ts, series = "DLNM - GLMM") +
  labs(
    x = "Time (weeks)",
    y = "Hospital discharge risk"
  ) +
...
autoplot(df_rn4$riesgos_ts, series = "Observed") +
  autolayer(df_rn4$glmm_ts, series = "GLMM") +
  autolayer(df_rn4$glm_dlnm_ts, series = "DLNM - GLMM") +
  labs(
    x = "Time (weeks)",
    y = "Hospital discharge risk"
  ) +
...
autoplot(df_rn5$riesgos_ts, series = "Observed") +
  autolayer(df_rn5$glmm_ts, series = "GLMM") +
  autolayer(df_rn5$glm_dlnm_ts, series = "DLNM - GLMM") +
  labs(
    x = "Time (weeks)",
    y = "Hospital discharge risk"
  ) +
...
coef_cond <- fixef(dlnm_glmm_model_rn)$cond
vcov_full <- vcov(dlnm_glmm_model_rn, full = TRUE)
...
save(
  ...
  coef_cond,
  vcov_full,
  file = "random_effects_models/dlnm_models_for_risk_rn.RData"
)
...
#' Subregion RN1
#' Create predictions for temperature with crosspred (DLNM-GLMM, subregion rn1)
...
#' Create predictions for precipitation with crosspred (DLNM-GLMM, subregion rn1)
...
#' Create predictions for relative humidity with crosspred (DLNM-GLMM, subregion rn1)
...
#' Create predictions for AOD with crosspred (DLNM-GLMM, subregion rn1)
...
#' Lag-response contour plot for mean temperature
plot(
  pred_dlnm_glmm_rn1_temp,
  "contour", # ← this is valid as first argument
  xlab = "Mean Temperature",
  ylab = "Lag",
  key.title = title("RR")
)

#' Lag-response plot at a specific temperature value (var = 30.3 °C)
plot(
  pred_dlnm_glmm_rn1_temp,
  "slices", # <- second unnamed argument
  var = max(pred_dlnm_glmm_rn1_temp$predvar), # specific predictor value
  col = 2,
  ylab = "RR"
)

#' Lag-response plot at another specific temperature value (var = 23.5 °C)
plot(
  pred_dlnm_glmm_rn1_temp,
  "slices",
...
#' Lag-response contour plot for mean precipitation
plot(
  pred_dlnm_glmm_rn1_prec,
  "contour",
  xlab = "Mean Precipitation",
...
#' Lag-response plot at a specific precipitation value (var = 20 mm)
plot(
  pred_dlnm_glmm_rn1_prec,
  "slices",
  var = 20,
...
#' Lag-response plot at another specific precipitation value (var = 40 mm)
plot(
  pred_dlnm_glmm_rn1_prec,
  "slices",
  var = 40,
...
#' Lag-response contour plot for relative humidity
plot(
  pred_dlnm_glmm_rn1_rh,
  "contour",
  xlab = "Relative Humidity",
...
#' Lag-response plot at a specific relative humidity value (var = 0.35)
plot(
  pred_dlnm_glmm_rn1_rh,
  "slices",
  var = 0.35,
...
#' Lag-response plot at another specific relative humidity value (var = 0.45)
plot(
  pred_dlnm_glmm_rn1_rh,
  "slices",
  var = 0.450,
...
#' 3D lag-response surface plot for AOD
plot(
...
#' Lag-response contour plot for AOD
plot(
  pred_dlnm_glmm_rn1_aod,
  "contour",
  xlab = "Mean AOD Index",
...
#' Lag-response plot at a specific AOD value (var = 0)
plot(
  pred_dlnm_glmm_rn1_aod,
  "slices",
  var = 0,
...
#' Lag-response plot at another specific AOD value (var = 4.9e-04)
plot(
  pred_dlnm_glmm_rn1_aod,
  "slices",
  var = max(pred_dlnm_glmm_rn1_aod$predvar),
  col = 2,
  ylab = "RR"
)
...
#' Create predictions for temperature using crosspred (DLNM-GLMM, subregion rn2)
...
#' Create predictions for precipitation using crosspred (DLNM-GLMM, subregion rn2)
...
#' Create predictions for relative humidity using crosspred (DLNM-GLMM, subregion rn2)
...
#' Create predictions for AOD using crosspred (DLNM-GLMM, subregion rn2)
...
#' Create predictions for temperature using crosspred (DLNM-GLMM, subregion rn3)
...
#' Create predictions for precipitation using crosspred (DLNM-GLMM, subregion rn3)
...
#' Create predictions for relative humidity using crosspred (DLNM-GLMM, subregion rn3)
...
#' Create predictions for AOD using crosspred (DLNM-GLMM, subregion rn3)
...
#' Create predictions for temperature using crosspred (DLNM-GLMM, subregion rn4)
...
#' Create predictions for precipitation using crosspred (DLNM-GLMM, subregion rn4)
...
#' Create predictions for relative humidity using crosspred (DLNM-GLMM, subregion rn4)
...
#' Create predictions for AOD using crosspred (DLNM-GLMM, subregion rn4)
...
#' Create predictions for temperature using crosspred (DLNM-GLMM, subregion rn5)
...
#' Create predictions for precipitation using crosspred (DLNM-GLMM, subregion rn5)
...
#' Create predictions for relative humidity using crosspred (DLNM-GLMM, subregion rn5)
...
#' Create predictions for AOD using crosspred (DLNM-GLMM, subregion rn5)
...
# Training dataset: excluding the forecast year
...
# Establish common factor levels for year in training and test datasets
...
# Prediction and forecast using DLNM-GLMM Normal model
df_train <- df_train %>%
...
# Prediction and forecast using GLMM model
df_train <- df_train %>%
