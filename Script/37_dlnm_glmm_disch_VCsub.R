...
library(dlnm) # Nonlinear relationships in time series models
library(forecast) # Time series forecasting
library(glmmTMB) # Generalized nonlinear mixed models
library(lme4) # Mixed effects models
library(mgcv) # Fitting GAM models
library(gamlss) # Fitting GAMLSS models with zero inflation
library(MASS) # GLM model fitting enhancements
...
#' The data include summarized values for discharges, population, precipitation,
#' temperature, aerosol concentration, and relative humidity.
...
# A column is added to identify each region, and all datasets are combined into one.
...
#' and the response variable in the VC region using different methods:
...
#' - method: character string, specifies dependence method: "pearson" (default), "dcor", or "MI".
...
#' lag nonlinear structures (DLNM), evaluating different types of
#' exposure and lag functions, with Poisson and negative binomial
#' regression models. The optimal model is selected based on
#' fit and prediction metrics.
#'
#' Parameters:
#' - data: data.frame with the input data
#' - variable: exposure variable name (string)
#' - respuesta: response variable name (string)
#' - max_lag: maximum lag value to evaluate
...
buscar_mejor_modelo <- function(data, variable, respuesta, max_lag, max_df_var, max_df_lag) {
  ...
  #' --------------------------------------------------------------------------
  #' Initial baseline evaluation: linear function and strata lag structure
  #' --------------------------------------------------------------------------
  ...
  #' --------------------------------------------------------------------------
  #' Evaluation using natural cubic spline for variable and lag
  #' --------------------------------------------------------------------------
  for (lag in 2:max_lag) {
...
max_lag <- 14
## 4.2 Correlational analysis in subregion VC1 ----
...
for (lag in 1:max_lag) {
  data_lag <- data_vc_subreg %>%
    filter(Subreg_climat == "VC1")
...
#' evaluation of univariate DLNM models for temperature in subregion VC1

data_vc1 <- data_vc_subreg %>%
  filter(Subreg_climat == "VC1")
...
#' compute correlations and p-values for each lag
for (lag in 1:max_lag) {
  data_vc1 <- data_vc_subreg %>% filter(Subreg_climat == "VC1")
  
  X_lagged <- head(data_vc1$precip_media, n = length(data_vc1$precip_media) - lag)
  Y_lagged <- tail(data_vc1$egreso_semana, n = length(data_vc1$egreso_semana) - lag)
...
#' evaluation of univariate DLNM models for precipitation in subregion VC1

res <- buscar_mejor_modelo(
  data_vc_subreg %>% filter(Subreg_climat == "VC1"), # Filter subregion VC1
...
for (lag in 1:max_lag) {
  df_filtered <- data_vc_subreg %>% filter(Subreg_climat == "VC1")
...
#' evaluation of univariate DLNM models for relative humidity in subregion VC1
...
for (lag in 1:max_lag) {
  datos <- data_vc_subreg %>% filter(Subreg_climat == "VC1")
...
#' evaluation of univariate DLNM models for AOD in subregion VC1
res <- buscar_mejor_modelo(
  data_vc_subreg %>% filter(Subreg_climat == "VC1"), # Filter subregion VC1
...
## 4.3 Correlational analysis in subregion VC2 ----
...
#' compute extreme values by metric
max_pearson <- max(results_temp$Pearson)
min_pearson <- min(results_temp$Pearson)
max_dcor <- max(results_temp$dCor)
min_dcor <- min(results_temp$dCor)
...
#' selection of best DLNM model for mean temperature in subregion VC2

#' filter the climatic subregion VC2 and fit DLNM models
res <- buscar_mejor_modelo(
  data = data_vc_subreg %>% filter(Subreg_climat == "VC2"), # Filter subregion VC2
...
#' view all evaluated models ordered by combined score
datatable(
  res$resultado_general,
  options = list(pageLength = 5, scrollX = TRUE)
) %>%
...
#' evaluation of univariate DLNM models for precipitation in subregion VC2
res <- buscar_mejor_modelo(
  data = data_vc_subreg %>% filter(Subreg_climat == "VC2"), # Filter subregion VC2
  variable = "precip_media", # Explanatory variable
  respuesta = "egreso_semana", # Response variable
...
for (lag in 1:max_lag) {
  data_lag <- data_vc_subreg %>% filter(Subreg_climat == "VC2")
...
## 4.4 Correlational analysis in subregion VC3 ----
...
for (lag in 1:max_lag) {
  X_lagged <- head(
    (data_vc_subreg %>% filter(Subreg_climat == "VC3"))$Temp_media,
    length((data_vc_subreg %>% filter(Subreg_climat == "VC3"))$Temp_media) - lag
  )
  Y_lagged <- tail(
    (data_vc_subreg %>% filter(Subreg_climat == "VC3"))$egreso_semana,
    length((data_vc_subreg %>% filter(Subreg_climat == "VC3"))$egreso_semana) - lag
  )
...
#' filter climatic subregion VC3

data_vc3 <- data_vc_subreg %>%
  filter(Subreg_climat == "VC3")
...
res <- buscar_mejor_modelo(
  data = data_vc_subreg %>% filter(Subreg_climat == "VC3"), # Filter subregion VC3
  variable = "RH", # Explanatory variable
  respuesta = "egreso_semana", # Response variable
  max_lag = 4, # Maximum lag
  max_df_var = 3, # Maximum df for the variable
...
## 4.5 Function for combined correlation analysis ----
...
#'
#' Details:
#' - The model fitted is of type glm.nb.
...
discrete_modeling_parameters <- function(
  ...
  # Determine the maximum lag across all crossbases to filter the dataset
    ...
    for (cb_prec in crossbases_filtradas$prec) {
      for (cb_rh in crossbases_filtradas$rh) {
        for (cb_aod in crossbases_filtradas$aod) {
          ...
          # MODEL (Poisson could be used for validation, behaves the same in parameters)
...
## 5. Construction of cross-bases by climatic subregion ----

#' This block defines the necessary crossbasis functions for DLNM-GLMM 
#' modeling of hospital discharges in subregions VC1, VC2, and VC3, based on 
#' climatic and atmospheric pollution variables.

#' sort dataset by climatic subregion

data_vc_subreg <- data_vc_subreg %>%
  arrange(Subreg_climat)

## 5.1 Crossbasis for subregion VC1 ----

cb_t1 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC1") %>% pull(Temp_media),
                    lag = 12, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_p1 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC1") %>% pull(precip_media),
                    lag = 3, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_rh1 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC1") %>% pull(RH),
                     lag = 4, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_aod1 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC1") %>% pull(aerosol),
                      lag = 13, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)

## 5.2 Crossbasis for subregion VC2 ----

cb_t2 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC2") %>% pull(Temp_media),
                    lag = 3, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_p2 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC2") %>% pull(precip_media),
                    lag = 6, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_rh2 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC2") %>% pull(RH),
                     lag = 5, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_aod2 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC2") %>% pull(aerosol),
                      lag = 12, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)

## 5.3 Crossbasis for subregion VC3 ----

cb_t3 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC3") %>% pull(Temp_media),
                    lag = 9, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_p3 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC3") %>% pull(precip_media),
                    lag = 5, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_rh3 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC3") %>% pull(RH),
                     lag = 4, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
cb_aod3 <- crossbasis(data_vc_subreg %>% filter(Subreg_climat == "VC3") %>% pull(aerosol),
                      lag = 10, argvar = list(fun = "ns", df = 3), arglag = list(fun = "ns", df = 3)
)
## 5.4 Merging cross-basis matrices -----
...
#' Remove initial observations by group according to the highest lag (lag = 13)
...
#' Get the names of columns generated by crossbasis
...
#' Percentage of zeros in subregions, in total
summary(data_vc_subreg$egreso_semana) #' no zeros detected
# 6. Poisson DLNM-GLMM Models ----

## 6.1 Simple Poisson GLMM Model ----

modelo_glmm_poi_vc <- glmmTMB(
...
modelo_dlnm_glmm_poi_vc_2 <- glmmTMB(
  egreso_semana ~
...
# 7. Negative Binomial DLNM-GLMM Models -----

## 7.1 Simple Negative Binomial GLMM Model ----

modelo_glmm_nb_vc <- glmmTMB(
  egreso_semana ~ Temp_media + precip_media + RH + aerosol +
    offset(logpop) + as.factor(epi.week),
  data = df_con_crossbasis_egresos,
...
modelo_dlnm_glmm_nb_vc_2 <- glmmTMB(
  egreso_semana ~
    T_v1.l1 + T_v1.l2 + T_v1.l3 +
    T_v2.l1 + T_v2.l2 + T_v2.l3 +
    T_v3.l1 + T_v3.l2 + T_v3.l3 +
...
#' (GLMM with different distributions), and builds a table sorted by the best 
#' fit score.

#' Order fitted values in the dataset
df_con_crossbasis_egresos_vc <- df_con_crossbasis_egresos %>%
  mutate(
    #' Poisson GLMM Models
    ajustado_glmm_poi = fitted(modelo_glmm_poi_vc),
    
    #' DLNM + Poisson GLMM Models
    ajustado_dlnm_glmm_poi_2 = fitted(modelo_dlnm_glmm_poi_vc_2),
    
    #' Negative Binomial GLMM Models
    ajustado_glmm_nb = fitted(modelo_glmm_nb_vc),
    
    #' DLNM + Negative Binomial GLMM Models
    ajustado_dlnm_glmm_nb_2 = fitted(modelo_dlnm_glmm_nb_vc_2)
  )
...
calcular_metricas <- function(modelo, predicciones, obs, nombre_modelo) {
  ...
  #' --------------------------------------------------------------------------
  #' Compute composite score: inverse metrics + adjusted logLik
  #' --------------------------------------------------------------------------
...
resultados_modelos <- rbind(
  calcular_metricas(
    modelo_glmm_poi_vc,
    df_con_crossbasis_egresos_vc$ajustado_glmm_poi,
    df_con_crossbasis_egresos_vc$egreso_semana,
    "GLMM_Poi"
  ),
  calcular_metricas(
    modelo_dlnm_glmm_poi_vc_2,
    df_con_crossbasis_egresos_vc$ajustado_dlnm_glmm_poi_2,
    df_con_crossbasis_egresos_vc$egreso_semana,
    "DLNM_GLMM_Poi_2"
  ),
  calcular_metricas(
    modelo_glmm_nb_vc,
    df_con_crossbasis_egresos_vc$ajustado_glmm_nb,
    df_con_crossbasis_egresos_vc$egreso_semana,
    "GLMM_NB"
  ),
  calcular_metricas(
    modelo_dlnm_glmm_nb_vc_2,
    df_con_crossbasis_egresos_vc$ajustado_dlnm_glmm_nb_2,
    df_con_crossbasis_egresos_vc$egreso_semana,
    "DLNM_GLMM_NB_2"
  )
)

#' Sort models from best to worst according to the composite score
...
dlnm_glmm_model_vc <- modelo_dlnm_glmm_nb_vc_2
summary(dlnm_glmm_model_vc)

## 8.3 Metrics for filtered subregions ----

#' Time series construction by model for subregion VC1
df_vc1 <- df_con_crossbasis_egresos_vc %>%
  filter(Subreg_climat == "VC1") %>%
  mutate(
    egresos_ts = ts(egreso_semana, start = c(2000, 14), frequency = 52),
    glmm_ts = ts(ajustado_glmm_nb, start = c(2000, 14), frequency = 52),
    glmm_dlnm_ts = ts(ajustado_dlnm_glmm_nb_2, start = c(2000, 14), frequency = 52)
  )

#' Time series construction for subregion VC2
df_vc2 <- df_con_crossbasis_egresos_vc %>%
  filter(Subreg_climat == "VC2") %>%
  mutate(
    egresos_ts = ts(egreso_semana, start = c(2000, 14), frequency = 52),
    glmm_ts = ts(ajustado_glmm_nb, start = c(2000, 14), frequency = 52),
...
#' Time series construction for subregion VC3
df_vc3 <- df_con_crossbasis_egresos_vc %>%
  filter(Subreg_climat == "VC3") %>%
  mutate(
    egresos_ts = ts(egreso_semana, start = c(2000, 14), frequency = 52),
    glmm_ts = ts(ajustado_glmm_nb, start = c(2000, 14), frequency = 52),
...
calcular_metricas_sub <- function(df, subregion, col_obs = "egresos_ts") {
  ...
  resultados <- lapply(columnas_modelo, function(nombre_modelo) {
    ...
    #' Composite score as the sum of inverses of error metrics
...
resultados_modelos <- rbind(
  calcular_metricas_sub(df_vc1, "VC1", "egreso_semana"),
  calcular_metricas_sub(df_vc2, "VC2", "egreso_semana"),
  calcular_metricas_sub(df_vc3, "VC3", "egreso_semana")
)
...
# 9. Residual analysis of the optimal model ----
...
df_con_crossbasis_egresos_vc <- df_con_crossbasis_egresos_vc %>%
  mutate(
    resid_pearson_vc = resid_pearson_vc,
    resid_deviance_vc = resid_deviance_vc
  )

## 9.1 Residual diagnostics ----

#' Q-Q plots to assess normality of residuals
qqnorm(resid_pearson_vc, main = "Pearson Residuals")
qqline(resid_pearson_vc, col = "red")

qqnorm(resid_deviance_vc, main = "Deviance Residuals")
...
resid_pearson_vc1 <- df_con_crossbasis_egresos_vc %>%
  filter(Subreg_climat == "VC1") %>%
  pull(resid_pearson_vc)

resid_pearson_vc2 <- df_con_crossbasis_egresos_vc %>%
  filter(Subreg_climat == "VC2") %>%
  pull(resid_pearson_vc)

resid_pearson_vc3 <- df_con_crossbasis_egresos_vc %>%
  filter(Subreg_climat == "VC3") %>%
  pull(resid_pearson_vc)

#' Extract deviance residuals by climatic subregion
resid_deviance_vc1 <- df_con_crossbasis_egresos_vc %>%
  filter(Subreg_climat == "VC1") %>%
  pull(resid_deviance_vc)

resid_deviance_vc2 <- df_con_crossbasis_egresos_vc %>%
  filter(Subreg_climat == "VC2") %>%
  pull(resid_deviance_vc)

resid_deviance_vc3 <- df_con_crossbasis_egresos_vc %>%
  filter(Subreg_climat == "VC3") %>%
  pull(resid_deviance_vc)

#' Time series diagnostic plots for Pearson residuals by subregion
ggtsdisplay(resid_pearson_vc1) # Subregion VC1
ggtsdisplay(resid_pearson_vc2) # Subregion VC2
ggtsdisplay(resid_pearson_vc3) # Subregion VC3

#' Time series diagnostic plots for deviance residuals by subregion
ggtsdisplay(resid_deviance_vc1) # Subregion VC1
ggtsdisplay(resid_deviance_vc2) # Subregion VC2
ggtsdisplay(resid_deviance_vc3) # Subregion VC3
# 10. Model predictions ----

## 10.1 Comparative plots of model fits ----

#' Comparative plots of predicted values - Subregion VC1
#' Comparison chart: observed vs. GLMM and DLNM-GLMM models
autoplot(df_vc1$egresos_ts, series = "Observed") +
  autolayer(df_vc1$glmm_ts, series = "GLMM NB") +
  autolayer(df_vc1$glmm_dlnm_ts, series = "DLNM - GLMM NB") +
  labs(
    x = "Time (weeks)",
...
#' Comparative plots of predicted values - Subregion VC2
#' Comparison chart: observed vs. GLMM and DLNM-GLMM models
autoplot(df_vc2$egresos_ts, series = "Observed") +
  autolayer(df_vc2$glmm_ts, series = "GLMM NB") +
  autolayer(df_vc2$glmm_dlnm_ts, series = "DLNM - GLMM NB") +
  labs(
    x = "Time (weeks)",
    y = "Hospital discharges"
  ) +
...
#' Comparative plots of predicted values - Subregion VC3
#' Comparison chart: observed vs. GLMM and DLNM-GLMM models
autoplot(df_vc3$egresos_ts, series = "Observed") +
  autolayer(df_vc3$glmm_ts, series = "GLMM NB") +
  autolayer(df_vc3$glmm_dlnm_ts, series = "DLNM - GLMM NB") +
  labs(
    x = "Time (weeks)",
    y = "Hospital discharges"
  ) +
...
## 10.2 Predictions ----
#' An example is presented for the behavior in one subregion.

coef_cond <- fixef(dlnm_glmm_model_vc)$cond
vcov_full <- vcov(dlnm_glmm_model_vc, full = TRUE)
...
save(
  ...
  coef_cond,
  vcov_full,
  file = "random_effects_models/dlnm_models_for_discharges_vc.RData"
)
load("random_effects_models/dlnm_models_for_discharges_vc.RData")

#' Extract coefficients and variance-covariance matrix for temperature (cb_t1 - subregion VC1)
...
#' Extract coefficients and variance-covariance matrix for precipitation (cb_p1 - subregion VC1)
...
#' Extract coefficients and variance-covariance matrix for relative humidity (cb_rh1 - subregion VC1)
...
#' Extract coefficients and variance-covariance matrix for aerosols (cb_aod1 - subregion VC1)
...
#' Subregion VC1 
#' Create predictions for temperature using crosspred (DLNM-GLMM, subregion VC1)
pred_dlnm_glmm_vc1_temp <- crosspred(
  cb_t1,
  coef = coef_t,
  vcov = vcov_t,
  model.link = "log",
...
#' Create predictions for precipitation using crosspred (DLNM-GLMM, subregion VC1)
pred_dlnm_glmm_vc1_prec <- crosspred(
  cb_p1,
  coef = coef_p,
  vcov = vcov_p,
  model.link = "log",
...
#' Create predictions for relative humidity using crosspred (DLNM-GLMM, subregion VC1)
pred_dlnm_glmm_vc1_rh <- crosspred(
  cb_rh1,
  coef = coef_rh,
  vcov = vcov_rh,
  model.link = "log",
...
#' Create predictions for AOD using crosspred (DLNM-GLMM, subregion VC1)
pred_dlnm_glmm_vc1_aod <- crosspred(
  cb_aod1,
  coef = coef_aod,
  vcov = vcov_aod,
  model.link = "log",
...
plot(
  pred_dlnm_glmm_vc1_temp,
  "contour", # ← this is a valid argument
  xlab = "Mean Temperature",
  ylab = "Lag",
  key.title = title("RR")
)

#' Lag-response plot for a specific temperature value (var = 26 °C)
plot(
  pred_dlnm_glmm_vc1_temp,
  "slices", # <- second unnamed argument
  var = 26, # specific value of the predictor variable
  col = 2,
  ylab = "RR"
)

#' Lag-response plot for another specific temperature value (var = 21 °C)
plot(
  pred_dlnm_glmm_vc1_temp,
  "slices",
  var = 21,
...
#' Plot of the overall cumulative effect for mean temperature
plot(
  pred_dlnm_glmm_vc1_temp,
  "overall",
  xlab = "Mean Temperature",
...
#' Lag-response plot for another specific relative humidity value (var = 0.40)
plot(
  pred_dlnm_glmm_vc1_rh,
  "slices",
  var = 0.40,
...
#' Generate predictions for temperature using crosspred (DLNM-GLMM, region VC2)
pred_dlnm_glmm_vc2_temp <- crosspred(
  cb_t2,
  coef = coef_t,
  vcov = vcov_t,
  model.link = "log",
...
#' Generate predictions for precipitation using crosspred (DLNM-GLMM, region VC2)
pred_dlnm_glmm_vc2_prec <- crosspred(
  cb_p2,
  coef = coef_p,
  vcov = vcov_p,
  model.link = "log",
...
#' Generate predictions for relative humidity using crosspred (DLNM-GLMM, region VC2)
pred_dlnm_glmm_vc2_rh <- crosspred(
  cb_rh2,
  coef = coef_rh,
  vcov = vcov_rh,
  model.link = "log",
...
#' Generate predictions for AOD using crosspred (DLNM-GLMM, region VC2)
pred_dlnm_glmm_vc2_aod <- crosspred(
  cb_aod2,
  coef = coef_aod,
  vcov = vcov_aod,
  model.link = "log",
...
#' Generate predictions for temperature using crosspred (DLNM-GLMM, region VC3)
pred_dlnm_glmm_vc3_temp <- crosspred(
  cb_t3,
  coef = coef_t,
  vcov = vcov_t,
  model.link = "log",
...
#' Generate predictions for precipitation using crosspred (DLNM-GLMM, region VC3)
pred_dlnm_glmm_vc3_prec <- crosspred(
  cb_p3,
  coef = coef_p,
  vcov = vcov_p,
  model.link = "log",
...
#' Generate predictions for relative humidity using crosspred (DLNM-GLMM, region VC3)
pred_dlnm_glmm_vc3_rh <- crosspred(
  cb_rh3,
  coef = coef_rh,
  vcov = vcov_rh,
  model.link = "log",
...
#' Generate predictions for AOD using crosspred (DLNM-GLMM, region VC3)
pred_dlnm_glmm_vc3_aod <- crosspred(
  cb_aod3,
  coef = coef_aod,
  vcov = vcov_aod,
  model.link = "log",
...
## 11.1 Forecast for the year 2017 ----
...
# Harmonize factor levels for year between train and test sets
...
# Fit the DLNM-GLMM Negative Binomial model
modelo_dlnm_glmm_nb <- glmmTMB(
  egreso_semana ~
    T_v1.l1 + T_v1.l2 + T_v1.l3 +
    T_v2.l1 + T_v2.l2 + T_v2.l3 +
    T_v3.l1 + T_v3.l2 + T_v3.l3 +
...
# Predictions and forecasts using DLNM-GLMM NB
df_train <- df_train %>%
  mutate(ajust_dlnm_glmm_nb = predict(modelo_dlnm_glmm_nb, type = "response"))

df_test <- df_test %>%
  mutate(pred_dlnm_glmm_nb = predict(modelo_dlnm_glmm_nb, newdata = df_test, type = "response"))
...
# Predictions and forecasts using GLMM
df_train <- df_train %>%
...
compute_errors <- function(obs, pred, model_name) {
  tibble(
    model = model_name,
    MAE = mae(obs, pred),
    MSE = mse(obs, pred),
...
errors_2017 <- bind_rows(
  compute_errors(df_train$egreso_semana, df_train$ajust_dlnm_glmm_nb, "DLNM_GLMM_train"),
  compute_errors(df_train$egreso_semana, df_train$ajust_glmm, "GLMM_train"),
  compute_errors(df_test_2017$egreso_semana, df_test_2017$pred_dlnm_glmm_nb, paste0("DLNM_GLMM_", forecast_year)),
  compute_errors(df_test_2017$egreso_semana, df_test_2017$pred_glmm, paste0("GLMM_", forecast_year))
)

## 11.2 Forecast for the year 2018 ----
...
# Harmonize factor levels for 'year' in both training and test sets
year_levels <- sort(unique(df_con_crossbasis_egresos$year))
...
# Predictions and forecast with DLNM-GLMM NB
df_train <- df_train %>%
  mutate(ajust_dlnm_glmm_nb = predict(modelo_dlnm_glmm_nb, type = "response"))
...
# Fit the standard GLMM model (glmmTMB)
modelo_glmm_nb <- glmmTMB(
  egreso_semana ~ Temp_media + precip_media + RH + aerosol +
    offset(logpop) + as.factor(epi.week),
  data = df_train,
...
# Predictions and forecast with GLMM
df_train <- df_train %>%
  mutate(ajust_glmm = predict(modelo_glmm_nb, type = "response"))
...
errors_2018 <- bind_rows(
  calcular_errores(df_train$egreso_semana,
                   df_train$ajust_dlnm_glmm_nb, "DLNM_GLMM_train"),
  calcular_errores(df_train$egreso_semana,
                   df_train$ajust_glmm, "GLMM_train"),
  calcular_errores(df_test_2018$egreso_semana,
                   df_test_2018$pred_dlnm_glmm_nb,
                   paste0("DLNM_GLMM_", forecast_year)),
  calcular_errores(df_test_2018$egreso_semana,
                   df_test_2018$pred_glmm, paste0("GLMM_", forecast_year))
)
## 11.3 Forecast for the year 2019 ----
...
# Harmonize factor levels for 'year' in both datasets
year_levels <- sort(unique(df_con_crossbasis_egresos$year))
...
# Fit DLNM-GLMM Negative Binomial model
modelo_dlnm_glmm_nb <- glmmTMB(
  egreso_semana ~
    T_v1.l1 + T_v1.l2 + T_v1.l3 +
    T_v2.l1 + T_v2.l2 + T_v2.l3 +
...
errors_2019 <- bind_rows(
  ...
  calcular_errores(df_train$egreso_semana,
                   df_train$ajust_glmm, "GLMM_train"),
  calcular_errores(df_test_2019$egreso_semana,
                   df_test_2019$pred_dlnm_glmm_nb,
                   paste0("DLNM_GLMM_", forecast_year)),
  calcular_errores(df_test_2019$egreso_semana,
                   df_test_2019$pred_glmm, paste0("GLMM_", forecast_year))
)

# Output the results
total_errors <- bind_rows(
  errores_2017,
  errores_2018,
  errors_2019
)
