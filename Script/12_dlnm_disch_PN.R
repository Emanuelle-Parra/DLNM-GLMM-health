...
library(ggplot2)     # Grammar of graphics (ggplot style)
library(ggseas)      # Seasonal adjustment visualizations
library(scales)      # Axis and scale customization
library(viridis)     # Perceptually uniform color palettes
library(corrplot)    # Correlation plots
library(GGally)      # Enhancements for ggplot2 (e.g., matrix plots)

## 1.3 Statistical analysis and modeling ----

library(car)         # Linear model diagnostics and tools
library(dlnm)        # Distributed lag non-linear models (DLNM)
library(forecast)    # Time series forecasting
library(glmmTMB)     # Generalized linear mixed models
library(lme4)        # Mixed-effects models
library(mgcv)        # Generalized additive models (GAMs)
library(gamlss)      # Generalized additive models for location, scale and shape (zero inflation)
library(MASS)        # GLM model fitting and support
library(splines)     # Cubic spline functions
library(urca)        # Unit root tests for time series
library(tseries)     # Time series analysis

## 1.4 Information metrics and analysis ----

library(energy)      # Distance correlation (dCor)
library(infotheo)    # Mutual information (MI)
library(Metrics)     # Performance metrics for prediction models
...
library(kableExtra)  # LaTeX/HTML table formatting
...
#' The dataset includes summarized values of discharges, relative risk of infection,
...
#' Add region label to each dataset and combine all regions into a single set
...
#' and the response variable in region PC using different dependency methods:
...
#' distributed non-linear lag structures (DLNM), testing different types of
#' exposure and lag functions, using Poisson and Negative Binomial regressions.
...
buscar_mejor_modelo <- function(data, variable, respuesta, max_lag, max_df_var, max_df_lag) {
  #' --------------------------------------------------------------------------
  #' Initialize structure to store model evaluation results
  #' --------------------------------------------------------------------------
  ...
  #' --------------------------------------------------------------------------
  #' Evaluation with cubic splines for exposure and lag functions
  #' --------------------------------------------------------------------------
  for (lag in 2:max_lag) {
...
for (lag in 1:max_lag) {
  ...
  Y_lagged <- tail(data_pn$egreso_semana, length(data_pn$egreso_semana) - lag)
...
#' Compute max and min values for each correlation method
...
for (lag in 1:max_lag) {
  # Shift the series to evaluate lagged effects
  X_lagged <- head(data_pn$precip_media, length(data_pn$precip_media) - lag)
  Y_lagged <- tail(data_pn$egreso_semana, length(data_pn$egreso_semana) - lag)
...
#' Compute max and min values for each correlation series
...
ggplot(results_RH, aes(x = Lag)) +
  ...
  #' Axis labels and color customization
  labs(
...
#' Evaluation of models for relative humidity (RH)
#' with a maximum of 8 lags, based on previous data
res <- buscar_mejor_modelo(
  data_pn,
  "RH",
  "egreso_semana",
...
#' Compute max and min for each correlation type
...
ggplot(results_AOD, aes(x = Lag)) +
  ...
  # Add annotations for max and min values in each method
  annotate("text",
    ...
    for (cb_prec in crossbases_filtradas$prec) {
      for (cb_rh in crossbases_filtradas$rh) {
        for (cb_aod in crossbases_filtradas$aod) {
          ...
          # MODEL (Poisson could be used for validation; parameters behave similarly)
...
d_temp <- data_pn %>%
  ...
  slice(15:n()) # Remove missing values due to lag
...
#' Scatter plot for precip_media (lag 6)

d_precip <- data_pn %>%
  mutate(precip_media_lag6 = lag(precip_media, 6)) %>%
  slice(7:n())

ggplot(d_precip, aes(x = precip_media_lag6, y = log(egreso_semana))) +
  geom_point(color = "#f03b20") + # Red color
  labs(x = "Mean precipitation (lag 6)", y = "Log of weekly discharges") +
  theme_bw()
...
#' Scatter plot for RH (lag 8)
d_rh <- data_pn %>%
  mutate(RH_lag8 = lag(RH, 8)) %>%
  slice(9:n())

ggplot(d_rh, aes(x = RH_lag8, y = log(egreso_semana))) +
  geom_point(color = "#756bb1") + # Purple color
  labs(x = "Relative humidity (lag 8)", y = "Log of weekly discharges") +
  theme_bw()
...
#' Different lags are set for each variable according to the data characteristics.
...
## 5.2 Mean precipitation (lag = 6, cubic spline) ----
...
## 5.3 Relative humidity (lag = 8, cubic spline) ----
...
#' This code block fits DLNM models using Poisson and negative binomial
#' distributions to evaluate the effect of climatic variables
#' on hospital discharges.

#' Remove the first 11 observations so that lags are complete
df_pn_fitted_egresos <- data_pn %>%
  slice(-(1:14))

#' ----------------------------------------------------
## 6.1 DLNM model with Poisson distribution and weekly factors ----
#' ----------------------------------------------------
dlnm_model_poi_estac_pn <- glm(
  egreso_semana ~ cb_temp_egre_pn + cb_prec_egre_pn +
...
df_pn_fitted_egresos <- df_pn_fitted_egresos %>%
  mutate(
    ajustado_dlnm_model_poi_estac_pn = fitted(dlnm_model_poi_estac_pn),
    ajustado_dlnm_model_poi_pn = fitted(dlnm_model_poi_pn),
    ajustado_dlnm_model_nb_estac_pn = fitted(dlnm_model_nb_estac_pn),
    ajustado_dlnm_model_nb_pn = fitted(dlnm_model_nb_pn),
    ajustado_glm_model_poi_pn = fitted(glm_model_poi_pn),
    ajustado_glm_model_nb_pn = fitted(glm_model_nb_pn)
  )
...
resultados <- rbind(
  calcular_metricas(
    dlnm_model_poi_estac_pn,
    df_pn_fitted_egresos$ajustado_dlnm_model_poi_estac_pn,
    df_pn_fitted_egresos$egreso_semana,
    "DLNM_Poi_est_pn"
  ),
  calcular_metricas(
    dlnm_model_poi_pn,
    df_pn_fitted_egresos$ajustado_dlnm_model_poi_pn,
    df_pn_fitted_egresos$egreso_semana,
    "DLNM_Poi_pn"
  ),
  calcular_metricas(
    dlnm_model_nb_estac_pn,
    df_pn_fitted_egresos$ajustado_dlnm_model_nb_estac_pn,
    df_pn_fitted_egresos$egreso_semana,
    "DLNM_NB_est_pn"
  ),
  calcular_metricas(
    dlnm_model_nb_pn,
    df_pn_fitted_egresos$ajustado_dlnm_model_nb_pn,
    df_pn_fitted_egresos$egreso_semana,
    "DLNM_NB_pn"
  ),
  calcular_metricas(
    glm_model_poi_pn,
    df_pn_fitted_egresos$ajustado_glm_model_poi_pn,
    df_pn_fitted_egresos$egreso_semana,
    "GLM_Poi_pn"
  ),
  calcular_metricas(
    glm_model_nb_pn,
    df_pn_fitted_egresos$ajustado_glm_model_nb_pn,
    df_pn_fitted_egresos$egreso_semana,
    "GLM_NB_pn"
  )
)
...
#' Display results using interactive table format
...
## 8.2 Optimal model ----
dlnm_model_pn <- dlnm_model_nb_estac_pn

# Save the models inside the folder
save(dlnm_model_nb_estac_pn,
     dlnm_model_nb_pn,
     dlnm_model_pn,
     dlnm_model_poi_estac_pn,
     dlnm_model_poi_pn,
     glm_model_nb_pn,
     glm_model_poi_pn,
     file = "fixed_effects_models/models_for_discharges_pn.RData"
)
...
#' seasonal factors, as well as a GLM model with NB distribution,
#' for hospital discharges in the PN region.
#' ----------------------------------------------------------------------------
...
df_pn_fitted_egresos <- df_pn_fitted_egresos %>%
  mutate(
    ...
    ajuste_DLNM = ts(ajustado_dlnm_model_nb_estac_pn, start = c(2000, 15), frequency = 52), # Fitted with seasonal factors
    ajuste_glm = ts(ajustado_glm_model_nb_pn, start = c(2000, 15), frequency = 52) # Fitted with GLM
  )

#' Plot with autoplot to compare observed vs fitted values
autoplot(df_pn_fitted_egresos$egresos_ts, series = "Observados") +
  
  #' Line for Poisson fit with seasonal factors
  autolayer(df_pn_fitted_egresos$ajuste_DLNM,
            series = "Ajuste DLNM BN"
  ) +
  
  #' Line for GLM fit with NB distribution
  autolayer(df_pn_fitted_egresos$ajuste_glm,
...
pred_dlnm_pn_temp <- crosspred(
  cb_temp_egre_pn,
  dlnm_model_pn,
  cen = mean(data_pn$Temp_media, na.rm = TRUE), # reference point (center)
...
pred_dlnm_pn_prec <- crosspred(
  cb_prec_egre_pn,
  dlnm_model_pn,
  cen = mean(data_pn$precip_media, na.rm = TRUE), # reference point (center)
  bylag = 0.1 # lag increments
)
...
pred_dlnm_pn_rh <- crosspred(
  cb_rh_egre_pn,
  dlnm_model_pn,
  cen = mean(data_pn$RH, na.rm = TRUE), # reference point (center)
  bylag = 0.1 # lag increments
)
...
pred_dlnm_pn_aod <- crosspred(
  cb_aod_egre_pn,
  dlnm_model_pn,
  cen = mean(data_pn$aerosol, na.rm = TRUE), # reference point (center)
  bylag = 0.1 # lag increments
)
...
#' ----------------------------------------------------
#' Lag-response plot for a specific temperature value (31.6°C)
#' ----------------------------------------------------
plot(
  pred_dlnm_pn_temp, "slices",
  var = max(pred_dlnm_pn_temp$predvar),
  col = 2,
...
plot(
  pred_dlnm_pn_temp, "slices",
  var = min(pred_dlnm_pn_temp$predvar),
  col = 2,
  ylab = "Relative risk (RR)"
)
...
#' ----------------------------------------------------
#' Lag-response plot for a specific precipitation value (39 mm)
#' ----------------------------------------------------
plot(
  pred_dlnm_pn_prec, "slices",
  var = max(pred_dlnm_pn_prec$predvar),
  col = 2,
  ylab = "Relative risk (RR)"
)
...
plot(
  pred_dlnm_pn_rh, "slices",
  var = max(pred_dlnm_pn_rh$predvar),
  col = 2,
  ylab = "Relative risk (RR)"
)
...
# Create full observed series
serie_real <- ts(df_con_crossbasis_egresos$egreso_semana, start = c(2000, 13), frequency = 52)

# Create forecast series as ts, insert NA where there's no prediction
