...
library(scales) # Customizing plot scales
...
library(dlnm) # Non-linear time series modeling
...
#' and the response variable in the RA region using different methods:
...
## 4.1 Functions to set the crossbasis parameterization ----
...
#'
#' Details:
#' - For "pearson", calculates |cor(x, y)| with random permutations of x.
...
#' non-linear structures (DLNM), evaluating different types of exposure and lag
#' functions, using Gamma and Normal regression models.
...
for (lag in 1:max_lag) {
  ...
  Y_lagged <- tail(data_ra$Riesgo_relativ_contagio,
                   length(data_ra$Riesgo_relativ_contagio) - lag)
...
for (lag in 1:max_lag) {
  # Shift the series to evaluate lagged effect
  X_lagged <- head(data_ra$precip_media,
                   length(data_ra$precip_media) - lag)
  Y_lagged <- tail(data_ra$Riesgo_relativ_contagio,
                   length(data_ra$Riesgo_relativ_contagio) - lag)
...
#' - Crossbasis bases are generated with the crossbasis function (dlnm package).
#' ------------------------------------------------------------------------------
continuous_modeling_parameters <- function(
  ...
  # R2 is excluded from the final score.
    ...
    for (cb_prec in crossbases_filtradas$prec) {
      for (cb_rh in crossbases_filtradas$rh) {
        for (cb_aod in crossbases_filtradas$aod) {
          ...
          # MODEL (another continuous function could be used here for validation; behavior is the same)
...
#' Scatter plot for precip_media (lag 3)

d_precip <- data_ra %>%
  mutate(precip_media_lag3 = lag(precip_media, 3)) %>%
  slice(4:n())

ggplot(d_precip, aes(x = precip_media_lag3, y = Riesgo_relativ_contagio)) +
  geom_point(color = "#f03b20") + # Red color
  labs(x = "Mean precipitation (lag 3)", y = "Relative risk of hospital discharges") +
  theme_bw()


#' Scatter plot for aerosol (lag 10)
d_aerosol <- data_ra %>%
  mutate(aerosol_lag10 = lag(aerosol, 10)) %>%
  slice(11:n())

ggplot(d_aerosol, aes(x = aerosol_lag10, y = Riesgo_relativ_contagio)) +
  geom_point(color = "#31a354") + # Green color
  labs(x = "AOD index (lag 10)", y = "Relative risk of hospital discharges") +
  theme_bw()


#' Scatter plot for RH (lag 3)
d_rh <- data_ra %>%
  mutate(RH_lag3 = lag(RH, 3)) %>%
  slice(4:n())

ggplot(d_rh, aes(x = RH_lag3, y = Riesgo_relativ_contagio)) +
  geom_point(color = "#756bb1") + # Purple color
  labs(x = "Relative humidity (lag 3)", y = "Relative risk of hospital discharges") +
  theme_bw()




# 5. Crossbasis for climate variables  ----------------------------

#' This block of code creates crossbases using cubic splines to model the
#' non-linear relationship between climate variables and the risks of
#' hospital discharges.
...
## 5.2 Mean precipitation (lag = 3, cubic spline) ----
...
## 5.3 Relative humidity (lag = 3, cubic spline) ----
...
## 5.4 Aerosol AOD index (lag = 10, cubic spline) ----
...
#' to evaluate the effect of climate variables on hospital discharge risks.

#' Remove the first 11 observations to ensure complete lags
df_ra_fitted_riesgo <- data_ra %>%
  slice(-(1:11))

#' ----------------------------------------------------
## 6.1 DLNM model with Gamma distribution and weekly factors ----
#' ----------------------------------------------------
dlnm_model_gamma_estac_ra <- glm(
  Riesgo_relativ_contagio ~ cb_temp_egre_ra + cb_prec_egre_ra +
...
#' ----------------------------------------------------
dlnm_model_normal_estac_ra <- glm(
  Riesgo_relativ_contagio ~ cb_temp_egre_ra + cb_prec_egre_ra +
    cb_rh_egre_ra + cb_aod_egre_ra +
    as.factor(epi.week),
  family = gaussian(link = "identity"),
...
df_ra_fitted_riesgo <- df_ra_fitted_riesgo %>%
  mutate(
    ajustado_dlnm_model_gamma_ra = fitted(dlnm_model_gamma_ra),
    ajustado_dlnm_model_gamma_estac_ra = fitted(dlnm_model_gamma_estac_ra),
    ajustado_dlnm_model_normal_ra = fitted(dlnm_model_normal_ra),
    ajustado_dlnm_model_normal_estac_ra = fitted(dlnm_model_normal_estac_ra),
    ajustado_glm_model_gamma_ra = fitted(glm_model_gamma_ra),
    ajustado_glm_model_normal_ra = fitted(glm_model_normal_ra)
  )
...
resultados <- rbind(
  calcular_metricas(
    dlnm_model_gamma_ra,
    df_ra_fitted_riesgo$ajustado_dlnm_model_gamma_ra,
    df_ra_fitted_riesgo$Riesgo_relativ_contagio,
    "DLNM_Gamma_RA"
  ),
  calcular_metricas(
    dlnm_model_gamma_estac_ra,
    df_ra_fitted_riesgo$ajustado_dlnm_model_gamma_estac_ra,
    df_ra_fitted_riesgo$Riesgo_relativ_contagio,
    "DLNM_Gamma_est_RA"
  ),
  calcular_metricas(
    dlnm_model_normal_ra,
    df_ra_fitted_riesgo$ajustado_dlnm_model_normal_ra,
    df_ra_fitted_riesgo$Riesgo_relativ_contagio,
    "DLNM_Normal_RA"
  ),
  calcular_metricas(
    dlnm_model_normal_estac_ra,
    df_ra_fitted_riesgo$ajustado_dlnm_model_normal_estac_ra,
    df_ra_fitted_riesgo$Riesgo_relativ_contagio,
    "DLNM_Normal_est_RA"
  ),
  calcular_metricas(
    glm_model_gamma_ra,
    df_ra_fitted_riesgo$ajustado_glm_model_gamma_ra,
    df_ra_fitted_riesgo$Riesgo_relativ_contagio,
    "GLM_Gamma_RA"
  ),
  calcular_metricas(
    glm_model_normal_ra,
    df_ra_fitted_riesgo$ajustado_glm_model_normal_ra,
    df_ra_fitted_riesgo$Riesgo_relativ_contagio,
    "GLM_Normal_RA"
  )
)
...
## 8.2 Optimal model ----
dlnm_model_ra <- dlnm_model_gamma_estac_ra

# Save the models inside the folder
save(dlnm_model_gamma_estac_ra,
     dlnm_model_gamma_ra,
     dlnm_model_normal_estac_ra,
     dlnm_model_normal_ra,
     dlnm_model_ra,
     glm_model_gamma_ra,
     glm_model_normal_ra,
     file = "fixed_effects_models/models_for_risk_ra.RData"
)
...
df_ra_fitted_riesgo <- df_ra_fitted_riesgo %>%
  mutate(
    riesgo_ts = ts(riesgos_ra, start = c(2000, 12), frequency = 52), # Observed risk series
    ajuste_DLNM = ts(ajustado_dlnm_model_gamma_estac_ra, start = c(2000, 12), frequency = 52), # Fit with seasonal factors
    ajuste_glm = ts(ajustado_glm_model_gamma_ra, start = c(2000, 12), frequency = 52) # GLM fit
  )

#' Plot with autoplot to compare observed vs fitted values
autoplot(df_ra_fitted_riesgo$riesgo_ts, series = "Observed") +
  
  #' Line for Poisson fit with seasonal factors
  autolayer(df_ra_fitted_riesgo$ajuste_DLNM,
            series = "DLNM fit"
  ) +
  
  #' Line for GLM fit with NB distribution
  autolayer(df_ra_fitted_riesgo$ajuste_glm,
            series = "GLM fit"
  ) +
  ...
  #' Custom colors using a consistent palette
  scale_color_manual(
    values = c(
...
#' using crossbasis for temperature, precipitation,
...
pred_dlnm_ra_temp <- crosspred(
  cb_temp_egre_ra,
  dlnm_model_ra,
  cen = median(data_ra$Temp_media, na.rm = TRUE), # reference point (center)
...
pred_dlnm_ra_prec <- crosspred(
  cb_prec_egre_ra,
  dlnm_model_ra,
  cen = median(data_ra$precip_media, na.rm = TRUE), # reference point (center)
  bylag = 0.1 # lag increments
)
...
pred_dlnm_ra_rh <- crosspred(
  cb_rh_egre_ra,
  dlnm_model_ra,
  cen = median(data_ra$RH, na.rm = TRUE), # reference point (center)
  bylag = 0.1 # lag increments
)
...
pred_dlnm_ra_aod <- crosspred(
  cb_aod_egre_ra,
  dlnm_model_ra,
  cen = median(data_ra$aerosol, na.rm = TRUE), # reference point (center)
  bylag = 0.1 # lag increments
)
...
#' ----------------------------------------------------
#' Lag-response plot for a specific temperature value (22.6°C)
#' ----------------------------------------------------
plot(
  pred_dlnm_ra_temp, "slices",
  var = min(pred_dlnm_ra_temp$predvar),
  col = 2,
  ylab = "Relative Risk (RR)"
)
...
df_con_crossbasis_riesgos <- cbind(data_ra,
                                   setNames(
                                     as.data.frame(cb_temp_egre_ra),
                                     make.names(paste0("t_", colnames(cb_temp_egre_ra)), unique = TRUE)),
                                   setNames(
                                     as.data.frame(cb_prec_egre_ra),
                                     make.names(paste0("p_", colnames(cb_prec_egre_ra)), unique = TRUE)),
                                   setNames(
                                     as.data.frame(cb_rh_egre_ra),
                                     make.names(paste0("rh_", colnames(cb_rh_egre_ra)), unique = TRUE)),
                                   setNames(
                                     as.data.frame(cb_aod_egre_ra),
                                     make.names(paste0("aod_", colnames(cb_aod_egre_ra)), unique = TRUE)))
...
# Split data into training and test sets (2017)
...
# Get crossbasis exposure column names
...
# GLM linear model without lags (excluding 2017)
...
# Split data into training and test sets (2018)
df_train <- df_con_crossbasis_riesgos %>% 
  filter(year != 2018)

df_2018 <- df_con_crossbasis_riesgos %>% 
  filter(year == 2018)
...
# Split data into training and test sets (2019)
df_train <- df_con_crossbasis_riesgos %>% 
  filter(year != 2019)

df_2019 <- df_con_crossbasis_riesgos %>% 
  filter(year == 2019)
...
# Create forecasts as ts, inserting NA where no forecast is available
