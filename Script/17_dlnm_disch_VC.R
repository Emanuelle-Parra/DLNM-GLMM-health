...
## 1.3 Statistical analysis and models ----

library(car) # Analysis and diagnostics for linear models
library(dlnm) # Non-linear modeling for time series
...
#' A column is added to identify each region and all the data are combined into a single dataset.
...
#' distributed nonlinear structures over time (DLNM), evaluating various types of
#' exposure and lag functions with Poisson and negative binomial regression models.
...
buscar_mejor_modelo <- function(data, variable, respuesta, max_lag, max_df_var, max_df_lag) {
  ...
  #' --------------------------------------------------------------------------
  #' Helper function to evaluate model fit metrics for a given model
  #' --------------------------------------------------------------------------
  ...
  #' --------------------------------------------------------------------------
  #' Initial base evaluation: linear function and strata for the lag
  #' --------------------------------------------------------------------------
...
for (lag in 1:max_lag) {
  ...
  Y_lagged <- tail(data_vc$egreso_semana, length(data_vc$egreso_semana) - lag)
...
for (lag in 1:max_lag) {
  ...
  X_lagged <- head(data_vc$precip_media, length(data_vc$precip_media) - lag)
  Y_lagged <- tail(data_vc$egreso_semana, length(data_vc$egreso_semana) - lag)
...
#' Model evaluation for Relative Humidity (RH) with a maximum of 9 lags, based on the previous data
res <- buscar_mejor_modelo(
  data_vc,
  "RH",
  "egreso_semana",
...
res <- buscar_mejor_modelo(
  ...
  max_df_var = 3, # Maximum complexity for splines of the variable
  max_df_lag = 3 # Maximum complexity for splines of the lag
)
...
discrete_modeling_parameters <- function(
  ...
  # Function to generate crossbasis using specified max lag
    ...
    for (cb_prec in crossbases_filtradas$prec) {
      for (cb_rh in crossbases_filtradas$rh) {
        for (cb_aod in crossbases_filtradas$aod) {
          ...
          # MODEL (Poisson can be used to validate, same parameter behavior)
...
d_temp <- data_vc %>%
  ...
  slice(13:n()) # Remove NA values due to lag

ggplot(d_temp, aes(x = Temp_media_lag12, y = log(egreso_semana))) +
  geom_point(color = "#2c7fb8") + # Blue color
  labs(x = "Mean Temperature (lag 12)", y = "Log weekly discharges") +
  theme_bw()

#' Scatter plot for precip_media (lag 5)
d_precip <- data_vc %>%
  mutate(precip_media_lag5 = lag(precip_media, 5)) %>%
  slice(6:n())

ggplot(d_precip, aes(x = precip_media_lag5, y = log(egreso_semana))) +
  geom_point(color = "#f03b20") + # Red color
  labs(x = "Mean Precipitation (lag 5)", y = "Log weekly discharges") +
  theme_bw()
...
ggplot(d_aerosol, aes(x = aerosol_lag11, y = log(egreso_semana))) +
  ...
  labs(x = "AOD Index (lag 11)", y = "Log weekly discharges") +
  theme_bw()

#' Scatter plot for RH (lag 5)
d_rh <- data_vc %>%
  mutate(RH_lag5 = lag(RH, 5)) %>%
  slice(6:n())

ggplot(d_rh, aes(x = RH_lag5, y = log(egreso_semana))) +
  geom_point(color = "#756bb1") + # Purple color
  labs(x = "Relative Humidity (lag 5)", y = "Log weekly discharges") +
  theme_bw()


# 5. Crossbasis creation for climate variables ----------------------------

#' This block creates crossbasis using cubic splines to model the nonlinear
#' relationship between climate variables and hospital discharges.
...
## 5.2 Mean precipitation (lag = 5, cubic spline) ----
...
## 5.4 AOD aerosol index (lag = 11, cubic spline) ----
...
#' distributions to evaluate the effect of climate variables on hospital discharges.

#' Remove the first 11 observations to complete the lags
df_vc_fitted_egresos <- data_vc %>%
  slice(-(1:12))

#' ----------------------------------------------------
## 6.1 DLNM model with Poisson distribution and weekly factors ----
#' ----------------------------------------------------
dlnm_model_poi_estac_vc <- glm(
  egreso_semana ~ cb_temp_egre_vc + cb_prec_egre_vc +
...
df_vc_fitted_egresos <- df_vc_fitted_egresos %>%
  mutate(
    ajustado_dlnm_model_poi_estac_vc = fitted(dlnm_model_poi_estac_vc),
    ajustado_dlnm_model_poi_vc = fitted(dlnm_model_poi_vc),
    ajustado_dlnm_model_nb_estac_vc = fitted(dlnm_model_nb_estac_vc),
    ajustado_dlnm_model_nb_vc = fitted(dlnm_model_nb_vc),
    ajustado_glm_model_poi_vc = fitted(glm_model_poi_vc),
    ajustado_glm_model_nb_vc = fitted(glm_model_nb_vc)
  )
...
resultados <- rbind(
  calcular_metricas(
    dlnm_model_poi_estac_vc,
    df_vc_fitted_egresos$ajustado_dlnm_model_poi_estac_vc,
    df_vc_fitted_egresos$egreso_semana,
    "DLNM_Poi_est_vc"
  ),
  calcular_metricas(
    dlnm_model_poi_vc,
    df_vc_fitted_egresos$ajustado_dlnm_model_poi_vc,
    df_vc_fitted_egresos$egreso_semana,
    "DLNM_Poi_vc"
  ),
  calcular_metricas(
    dlnm_model_nb_estac_vc,
    df_vc_fitted_egresos$ajustado_dlnm_model_nb_estac_vc,
    df_vc_fitted_egresos$egreso_semana,
    "DLNM_NB_est_vc"
  ),
  calcular_metricas(
    dlnm_model_nb_vc,
    df_vc_fitted_egresos$ajustado_dlnm_model_nb_vc,
    df_vc_fitted_egresos$egreso_semana,
    "DLNM_NB_vc"
  ),
  calcular_metricas(
    glm_model_poi_vc,
    df_vc_fitted_egresos$ajustado_glm_model_poi_vc,
    df_vc_fitted_egresos$egreso_semana,
    "GLM_Poi_vc"
  ),
  calcular_metricas(
    glm_model_nb_vc,
    df_vc_fitted_egresos$ajustado_glm_model_nb_vc,
    df_vc_fitted_egresos$egreso_semana,
    "GLM_NB_vc"
  )
)
...
## 8.2 Optimal model ----
dlnm_model_vc <- dlnm_model_nb_estac_vc

# Save the models inside the folder
save(dlnm_model_nb_estac_vc,
     dlnm_model_nb_vc,
     dlnm_model_vc,
     dlnm_model_poi_estac_vc,
     dlnm_model_poi_vc,
     glm_model_nb_vc,
     glm_model_poi_vc,
     file = "fixed_effects_models/models_for_discharges_vc.RData"
)
...
#' and fitted values using DLNM Poisson models with and without seasonal factors,
#' as well as a GLM model with NB distribution, for hospital discharges
#' in the VC region.
#' ----------------------------------------------------------------------------
...
df_vc_fitted_egresos <- df_vc_fitted_egresos %>%
  mutate(
    ...
    ajuste_DLNM = ts(ajustado_dlnm_model_nb_estac_vc, start = c(2000, 13), frequency = 52), # DLNM with seasonal factors
    ajuste_glm = ts(ajustado_glm_model_nb_vc, start = c(2000, 13), frequency = 52) # GLM fit
  )

#' Plot using autoplot to compare observed vs fitted values
autoplot(df_vc_fitted_egresos$egresos_ts, series = "Observed") +
  
  #' Line for Poisson fit with seasonal factors
  autolayer(df_vc_fitted_egresos$ajuste_DLNM,
            series = "DLNM NB Fit"
  ) +
  
  #' Line for GLM NB distribution fit
  autolayer(df_vc_fitted_egresos$ajuste_glm,
            series = "GLM NB Fit"
  ) +
...
pred_dlnm_vc_temp <- crosspred(
  cb_temp_egre_vc,
  dlnm_model_vc,
  cen = mean(data_vc$Temp_media, na.rm = TRUE), # reference value (center)
...
pred_dlnm_vc_prec <- crosspred(
  cb_prec_egre_vc,
  dlnm_model_vc,
  cen = mean(data_vc$precip_media, na.rm = TRUE), # reference value (center)
  bylag = 0.1
)
...
pred_dlnm_vc_rh <- crosspred(
  cb_rh_egre_vc,
  dlnm_model_vc,
  cen = mean(data_vc$RH, na.rm = TRUE),
  bylag = 0.1
)
...
pred_dlnm_vc_aod <- crosspred(
  cb_aod_egre_vc,
  dlnm_model_vc,
  cen = mean(data_vc$aerosol, na.rm = TRUE),
  bylag = 0.1
)
...
# Function to compute error metrics: MAE, MSE, RMSE, MAPE, SAMPE
...
# Error metrics for 2017 forecasts
...
# Create forecast series with NA where missing
