...
library(MASS)       # GLM-related utilities
...
library(Metrics)    # Metrics for model evaluation
...
#' The dataset includes summarized values of hospital discharges, relative risk of infection,
...
#' A column is added to identify each region, and all regional data are combined into a single dataset.
...
#' Define maximum lag to be evaluated
...
#' - method: character, dependence method to use: "pearson" (default), "dcor", or "MI"
...
#'
#' Details:
#' - For "pearson", |cor(x, y)| is computed over random permutations of x
#' - For "dcor", dcor() from the energy package is used
#' - For "MI", mutual information is computed from discretized values using mutinformation() from infotheo
...
#' distributed nonlinear structures (DLNM), testing different types of
#' exposure and lag functions using Poisson and Negative Binomial models.
...
buscar_mejor_modelo <- function(data, variable, respuesta, max_lag, max_df_var, max_df_lag) {
  #' --------------------------------------------------------------------------
  #' Create structure to store model evaluation results
  #' --------------------------------------------------------------------------
  ...
  #' --------------------------------------------------------------------------
  #' Helper function to evaluate performance metrics of a given model
  #' --------------------------------------------------------------------------
  ...
  #' --------------------------------------------------------------------------
  #' Initial evaluation: linear function with strata for the lag
  #' --------------------------------------------------------------------------
  ...
  #' --------------------------------------------------------------------------
  #' Identify model with the highest global score
  #' --------------------------------------------------------------------------
  mejor_modelo <- resultados[resultados$score == max(resultados$score), ]
  
  cat("\nBest model based on global score:\n")
  print(mejor_modelo)
...
for (lag in 1:max_lag) {
  ...
  Y_lagged <- tail(data_ps$egreso_semana, length(data_ps$egreso_semana) - lag)
...
#' Run best model search up to lag 14
...
#' Visualize results using DT::datatable
...
for (lag in 1:max_lag) {
  ...
  X_lagged <- head(data_ps$precip_media, length(data_ps$precip_media) - lag)
  Y_lagged <- tail(data_ps$egreso_semana, length(data_ps$egreso_semana) - lag)
...
ggplot(results_Prec, aes(x = Lag)) +
  ...
  # Show integer tick marks on X-axis
  scale_x_continuous(breaks = seq(1, max_lag, by = 1)) +
...
#' Plot correlation coefficients by lag
...
ggplot(results_RH, aes(x = Lag)) +
  ...
  #' Customize labels and axis scales
  labs(
...
#' Model evaluation for relative humidity (RH) with a maximum of 4 lags
...
#' Compute max and min values for each metric
...
ggplot(results_AOD, aes(x = Lag)) +
  ...
  #' Add annotations for max/min values of each method
  annotate("text",
...
#' - max_df_var_*: maximum df for the exposure function.
#' - max_df_lag_*: maximum df for the lag function.
#'
#' Output:
#' - A list containing a data.frame called 'resultados_completos' that contains
...
#' - Cross-bases are generated with the crossbasis function (dlnm package).
#' ------------------------------------------------------------------------------
...
discrete_modeling_parameters <- function(
  ...
  # The R2 term is excluded from the score
  ...
  # Obtain the maximum lag from all crossbases to filter the dataset
    ...
    for (cb_prec in crossbases_filtradas$prec) {
      for (cb_rh in crossbases_filtradas$rh) {
        for (cb_aod in crossbases_filtradas$aod) {
          ...
          # MODEL (Poisson could be used for validation, behavior is the same for parameters)
...
ggplot(d_temp, aes(x = Temp_media_lag11, y = log(egreso_semana))) +
  geom_point(color = "#2c7fb8") + # Blue color
  labs(x = "Mean temperature (lag 11)", y = "Log of weekly discharges") +
  theme_bw()


#' Scatter plot for precip_media (lag 7)

d_precip <- data_ps %>%
  mutate(precip_media_lag7 = lag(precip_media, 7)) %>%
  slice(8:n())

ggplot(d_precip, aes(x = precip_media_lag7, y = log(egreso_semana))) +
  geom_point(color = "#f03b20") + # Red color
  labs(x = "Mean precipitation (lag 7)", y = "Log of weekly discharges") +
  theme_bw()
...
ggplot(d_aerosol, aes(x = aerosol_lag13, y = log(egreso_semana))) +
  geom_point(color = "#31a354") + # Green color
  labs(x = "AOD index (lag 13)", y = "Log of weekly discharges") +
  theme_bw()
...
#' This code block creates cross-basis matrices using cubic splines to model
#' the nonlinear relationship between climatic variables and hospital discharges.
...
## 5.2 Mean precipitation (lag = 7, cubic spline) ----
...
## 5.4 Aerosol AOD index (lag = 13, cubic spline) ----
...
#' Removes the first 11 observations so that lags are complete
df_ps_fitted_egresos <- data_ps %>%
  slice(-(1:13))

#' ----------------------------------------------------
## 6.1 DLNM model with Poisson distribution and weekly factors ----
#' ----------------------------------------------------
dlnm_model_poi_estac_ps <- glm(
  egreso_semana ~ cb_temp_egre_ps + cb_prec_egre_ps +
...
df_ps_fitted_egresos <- df_ps_fitted_egresos %>%
  mutate(
    ajustado_dlnm_model_poi_estac_ps = fitted(dlnm_model_poi_estac_ps),
    ajustado_dlnm_model_poi_ps = fitted(dlnm_model_poi_ps),
    ajustado_dlnm_model_nb_estac_ps = fitted(dlnm_model_nb_estac_ps),
    ajustado_dlnm_model_nb_ps = fitted(dlnm_model_nb_ps),
    ajustado_glm_model_poi_ps = fitted(glm_model_poi_ps),
    ajustado_glm_model_nb_ps = fitted(glm_model_nb_ps)
  )
...
calcular_metricas <- function(modelo, predicciones, obs, nombre_modelo) {
  ...
  #' --------------------------------------------------------------------------
  #' Calculation of composite score: inverses + adjusted logLik
  #' --------------------------------------------------------------------------
...
resultados <- rbind(
  calcular_metricas(
    dlnm_model_poi_estac_ps,
    df_ps_fitted_egresos$ajustado_dlnm_model_poi_estac_ps,
    df_ps_fitted_egresos$egreso_semana,
    "DLNM_Poi_est_PS"
  ),
  calcular_metricas(
    dlnm_model_poi_ps,
    df_ps_fitted_egresos$ajustado_dlnm_model_poi_ps,
    df_ps_fitted_egresos$egreso_semana,
    "DLNM_Poi_PS"
  ),
  calcular_metricas(
    dlnm_model_nb_estac_ps,
    df_ps_fitted_egresos$ajustado_dlnm_model_nb_estac_ps,
    df_ps_fitted_egresos$egreso_semana,
    "DLNM_NB_est_PS"
  ),
  calcular_metricas(
    dlnm_model_nb_ps,
    df_ps_fitted_egresos$ajustado_dlnm_model_nb_ps,
    df_ps_fitted_egresos$egreso_semana,
    "DLNM_NB_PS"
  ),
  calcular_metricas(
    glm_model_poi_ps,
    df_ps_fitted_egresos$ajustado_glm_model_poi_ps,
    df_ps_fitted_egresos$egreso_semana,
    "GLM_Poi_PS"
  ),
  calcular_metricas(
    glm_model_nb_ps,
    df_ps_fitted_egresos$ajustado_glm_model_nb_ps,
    df_ps_fitted_egresos$egreso_semana,
    "GLM_NB_PS"
  )
)
...
## 8.2 Optimal model ----
dlnm_model_ps <- dlnm_model_nb_estac_ps

# Save the models inside the folder
save(dlnm_model_nb_estac_ps,
     dlnm_model_nb_ps,
     dlnm_model_ps,
     dlnm_model_poi_estac_ps,
     dlnm_model_poi_ps,
     glm_model_nb_ps,
     glm_model_poi_ps,
     file = "fixed_effects_models/models_for_discharges_ps.RData"
)
...
#' ACF and PACF plot of Pearson residuals
ggtsdisplay(resid_pearson_st)

#' ACF and PACF plot of deviance residuals
...
qqnorm(resid_pearson_ps, main = "QQ plot of Pearson residuals")
qqline(resid_pearson_ps, col = "red")

#' QQ plot for deviance residuals
qqnorm(resid_deviance_ps, main = "QQ plot of deviance residuals")
...
#' ----------------------------------------------------------------------------
#' This code generates a comparison plot between observed values
#' and fitted values using DLNM Poisson models with and without
#' seasonal factors, as well as the GLM model with NB distribution,
#' for hospital discharges in region PS.
#' ----------------------------------------------------------------------------

#' Create time series for the plot
df_ps_fitted_egresos <- df_ps_fitted_egresos %>%
  mutate(
    egresos_ts = ts(egreso_semana, start = c(2000, 14), frequency = 52), # Observed discharges series
    ajuste_DLNM = ts(ajustado_dlnm_model_nb_estac_ps, start = c(2000, 14), frequency = 52), # DLNM fit with seasonal factors
    ajuste_glm = ts(ajustado_glm_model_nb_ps, start = c(2000, 14), frequency = 52) # GLM fit
  )

#' Plot with autoplot to compare observed vs fitted values
autoplot(df_ps_fitted_egresos$egresos_ts, series = "Observados") +
  
  #' Line for Poisson fit with seasonal factors
  autolayer(df_ps_fitted_egresos$ajuste_DLNM,
            series = "Ajuste DLNM BN"
  ) +
  
  #' Line for GLM fit with NB distribution
  autolayer(df_ps_fitted_egresos$ajuste_glm,
            series = "Ajuste GLM BN"
  ) +
  ...
  scale_color_manual(
    values = c(
      "Observados" = "green", # Green
      "Ajuste DLNM BN" = "blue", # Blue
      "Ajuste GLM BN" = "red" # Red
    )
  ) +
...
## 10.2 Lagged effects predictions  ----
...
pred_dlnm_ps_temp <- crosspred(
  cb_temp_egre_ps,
  dlnm_model_ps,
  cen = mean(data_ps$Temp_media, na.rm = TRUE), # reference point (center)
...
pred_dlnm_ps_prec <- crosspred(
  cb_prec_egre_ps,
  dlnm_model_ps,
  cen = mean(data_ps$precip_media, na.rm = TRUE), # reference point (center)
  bylag = 0.1 # lag increments
)
...
pred_dlnm_ps_rh <- crosspred(
  cb_rh_egre_ps,
  dlnm_model_ps,
  cen = mean(data_ps$RH, na.rm = TRUE), # reference point (center)
  bylag = 0.1 # lag increments
)
...
pred_dlnm_ps_aod <- crosspred(
  cb_aod_egre_ps,
  dlnm_model_ps,
  cen = mean(data_ps$aerosol, na.rm = TRUE), # reference point (center)
  bylag = 0.1 # lag increments
)
...
#' ----------------------------------------------------
#' Lag-response plot for a specific temperature value (25.8°C)
#' ----------------------------------------------------
plot(
  pred_dlnm_ps_temp, "slices",
...
#' ----------------------------------------------------
#' Lag-response plot for a specific precipitation value (25 mm)
#' ----------------------------------------------------
plot(
...
#' ----------------------------------------------------
#' Lag-response plot for a specific precipitation value (30 mm)
#' ----------------------------------------------------
plot(
  pred_dlnm_ps_prec, "slices",
  var = 30,
...
#' ----------------------------------------------------
#' Lag-response plot for a specific humidity value (0.445)
#' ----------------------------------------------------
plot(
  pred_dlnm_ps_rh, "slices",
...
#' ----------------------------------------------------
#' Lag-response plot for a specific AOD value (4.8e-04)
#' ----------------------------------------------------
plot(
  pred_dlnm_ps_aod, "slices",
...
# Construction of df with crossbasis
df_con_crossbasis_egresos <- cbind(data_ps,
                                   setNames(
                                     as.data.frame(cb_temp_egre_ps),
                                     make.names(paste0("t_", colnames(cb_temp_egre_ps)), unique = TRUE)),
                                   setNames(
                                     as.data.frame(cb_prec_egre_ps),
                                     make.names(paste0("p_", colnames(cb_prec_egre_ps)), unique = TRUE)),
                                   setNames(
                                     as.data.frame(cb_rh_egre_ps),
                                     make.names(paste0("rh_", colnames(cb_rh_egre_ps)), unique = TRUE)),
                                   setNames(
                                     as.data.frame(cb_aod_egre_ps),
                                     make.names(paste0("aod_", colnames(cb_aod_egre_ps)), unique = TRUE)))
...
# Split dataset into training and testing (2017)
...
# NB linear model without lags (excluding 2017)
...
# Split dataset into training and testing (2018)
df_train <- df_con_crossbasis_egresos %>%
  filter(year != 2018)
...
# NB linear model without lags (excluding 2018)
glm_model_nb_pc <- glm.nb(
  egreso_semana ~ Temp_media + precip_media + RH + aerosol + as.factor(epi.week),
  data = df_train
)
...
# Forecast errors 2018
errores_2018 <- bind_rows(
  calcular_errores(df_train$egreso_semana, df_train$pred_glm_train, "GLM_train"),
  calcular_errores(df_train$egreso_semana, df_train$pred_dlnm_train, "DLNM_train"),
  calcular_errores(df_2018$egreso_semana, df_2017$pred_dlnm_2017, "DLNM_2018"),
  calcular_errores(df_2018$egreso_semana, df_2017$pred_glm_2017, "GLM_2018")
)

## 11.4 Forecast for 2019 ----

# Split dataset into training and testing (2019)
df_train <- df_con_crossbasis_egresos %>%
  filter(year != 2019)
...
# NB linear model without lags (excluding 2019)
glm_model_nb_pc <- glm.nb(
  egreso_semana ~ Temp_media + precip_media + RH + aerosol + as.factor(epi.week),
  data = df_train
)
...
# Forecast errors 2019
errores_2019 <- bind_rows(
  calcular_errores(df_train$egreso_semana, df_train$pred_glm_train, "GLM_train"),
  calcular_errores(df_train$egreso_semana, df_train$pred_dlnm_train, "DLNM_train"),
  calcular_errores(df_2019$egreso_semana, df_2017$pred_dlnm_2017, "DLNM_2019"),
  calcular_errores(df_2019$egreso_semana, df_2017$pred_glm_2017, "GLM_2019")
)
...
# Create forecast series as ts, keeping NA where there are no predictions
