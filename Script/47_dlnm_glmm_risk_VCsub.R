...
## 4.2 Correlation analysis in subregion VC1 ----
...
ggplot(results_temp, aes(x = Lag)) +
  ...
  # maximum and minimum annotations
  annotate("text",
...
for (lag in 1:max_lag) {
  ...
  Y_lagged <- tail(data_vc1$Riesgo_relativ_contagio,
                   n = length(data_vc1$Riesgo_relativ_contagio) - lag)
...
res <- buscar_mejor_modelo(
  data_vc_subreg %>% filter(Subreg_climat == "VC1"), # Filter for subregion VC1
...
## 4.3 Correlation analysis in subregion VC2 ----
...
#' DLNM model selection for mean temperature in subregion VC2

#' filter climate subregion VC2 and fit DLNM models
res <- buscar_mejor_modelo(
...
## 4.4 Correlation analysis in subregion VC3 ----
...
for (lag in 1:max_lag) {
  ...
  Y_lagged <- tail(
    (data_vc_subreg %>% filter(Subreg_climat == "VC3"))$Riesgo_relativ_contagio,
    length((data_vc_subreg %>% filter(Subreg_climat == "VC3"))$Riesgo_relativ_contagio) - lag
  )
...
#'
#' Output:
#' - List containing a data.frame named 'resultados_completos' with fit metrics
#'   (AIC, BIC, MAE, MSE, RMSE, R2, MAPE, logLik, score) for each parameter
#'   combination evaluated.
#'
#' Details:
#' - The fitted model is a glm
#' - Includes seasonal effects via as.factor(epi.week).
#' - Uses cross-bases generated with the crossbasis function (dlnm package).
#' ------------------------------------------------------------------------------
...
# 5. Construction of cross-bases by climate subregion ----

#' This block defines the necessary cross-basis functions for DLNM-GLMM
#' modeling of hospitalization risks in the subregions VC1, VC2, and VC3,
#' based on climatic and air pollution variables.

#' sort dataset by climate subregion
...
## 5.3 Crossbasis for subregion VC2 ----
...
## 5.4 Crossbasis for subregion VC3 ----
...
## 5.5 Merge cross-bases -----

#' combine data with crossbasis columns
...
#' remove initial observations per group based on the largest lag (lag = 14)
...
#' get names of exposure columns created by crossbasis
...
## Zero-inflation requirement in Gamma distribution models ----

#' proportion of zeros in subregions, overall
summary(data_vc_subreg$Riesgo_relativ_contagio)
# 6. DLNM-GLMM Models ----

## 6.1 GLMM Normal Model (1): simple ----

modelo_glmm_normal_vc <- glmmTMB(
  Riesgo_relativ_contagio ~ Temp_media + precip_media + RH + aerosol,
  data = df_con_crossbasis_riesgos,
  family = gaussian(link = "identity")
)

## 6.2 DLNM + GLMM Normal Model (2): with lags ----

modelo_dlnm_glmm_normal_vc <- glmmTMB(
...
## 6.3 GLMM Gamma Model (1): simple ----

modelo_glmm_gamma_vc <- glmmTMB(
  Riesgo_relativ_contagio ~ Temp_media + precip_media + RH + aerosol,
  data = df_con_crossbasis_riesgos,
  family = Gamma(link = "log")
)

## 6.4 DLNM + GLMM Gamma Model (2): with lags ----

modelo_dlnm_glmm_gamma_vc <- glmmTMB(
  Riesgo_relativ_contagio ~ 
    T_v1.l1 + T_v1.l2 + T_v1.l3 + 
    T_v2.l1 + T_v2.l2 + T_v2.l3 + 
    T_v3.l1 + T_v3.l2 + T_v3.l3 + 
...
#' (GLMM with different distributions), and constructs a table sorted by the
#' best fit score.

#' Sort fitted values in the dataset
df_con_crossbasis_riesgos_vc <- df_con_crossbasis_riesgos %>%
  mutate(
    #' Fitted values from GLMM Normal and Gamma models
    ajustado_modelo_glmm_normal_vc = fitted(modelo_glmm_normal_vc),
    ajustado_modelo_glmm_gamma_vc = fitted(modelo_glmm_gamma_vc),
    
    #' Fitted values from DLNM + GLMM Normal and Gamma models
    ajustado_modelo_dlnm_glmm_normal_vc = fitted(modelo_dlnm_glmm_normal_vc),
    ajustado_modelo_dlnm_glmm_gamma_vc = fitted(modelo_dlnm_glmm_gamma_vc)
  )
...
#' calcular_metricas()
#' ------------------------------------------------------------------------------
#' Function to calculate performance metrics for a fitted model and its associated
...
calcular_metricas <- function(modelo, predicciones, obs, nombre_modelo) {
  ...
  #' MAPE - includes division by zero correction
  epsilon <- 1e-10
  ...
  #' --------------------------------------------------------------------------
  #' Composite score calculation: inverse metrics + adjusted logLik
  #' --------------------------------------------------------------------------
...
resultados_modelos <- rbind(
  calcular_metricas(
    modelo_glmm_normal_vc,
    df_con_crossbasis_riesgos_vc$ajustado_modelo_glmm_normal_vc,
    df_con_crossbasis_riesgos_vc$Riesgo_relativ_contagio,
    "GLMM_Normal"
  ),
  calcular_metricas(
    modelo_dlnm_glmm_normal_vc,
    df_con_crossbasis_riesgos_vc$ajustado_modelo_dlnm_glmm_normal_vc,
    df_con_crossbasis_riesgos_vc$Riesgo_relativ_contagio,
    "DLNM_GLMM_Normal"
  ),
  calcular_metricas(
    modelo_glmm_gamma_vc,
    df_con_crossbasis_riesgos_vc$ajustado_modelo_glmm_gamma_vc,
    df_con_crossbasis_riesgos_vc$Riesgo_relativ_contagio,
    "GLMM_Gamma"
  ),
  calcular_metricas(
    modelo_dlnm_glmm_gamma_vc,
    df_con_crossbasis_riesgos_vc$ajustado_modelo_dlnm_glmm_gamma_vc,
    df_con_crossbasis_riesgos_vc$Riesgo_relativ_contagio,
    "DLNM_GLMM_Gamma"
  )
)
...
dlnm_glmm_model_vc <- modelo_dlnm_glmm_normal_vc

## 7.3 Metrics for filtered subregions ----

#' Construct time series by model for VC1
df_vc1 <- df_con_crossbasis_riesgos_vc %>%
  filter(Subreg_climat == "VC1") %>%
  mutate(
    riesgos_ts = ts(Riesgo_relativ_contagio, start = c(2000, 14), frequency = 52),
    glmm_ts = ts(ajustado_modelo_glmm_normal_vc, start = c(2000, 14), frequency = 52),
    glm_dlnm_ts = ts(ajustado_modelo_dlnm_glmm_normal_vc, start = c(2000, 14), frequency = 52)
  )

#' Construct time series by model for VC2
df_vc2 <- df_con_crossbasis_riesgos_vc %>%
  filter(Subreg_climat == "VC2") %>%
  mutate(
    riesgos_ts = ts(Riesgo_relativ_contagio, start = c(2000, 14), frequency = 52),
    glmm_ts = ts(ajustado_modelo_glmm_normal_vc, start = c(2000, 14), frequency = 52),
...
#' Construct time series by model for VC3
df_vc3 <- df_con_crossbasis_riesgos_vc %>%
  filter(Subreg_climat == "VC3") %>%
  mutate(
    riesgos_ts = ts(Riesgo_relativ_contagio, start = c(2000, 13), frequency = 52),
    glmm_ts = ts(ajustado_modelo_glmm_normal_vc, start = c(2000, 13), frequency = 52),
    glm_dlnm_ts = ts(ajustado_modelo_dlnm_glmm_normal_vc, start = c(2000, 13), frequency = 52)
  )

calcular_metricas_sub <- function(df, subregion, col_obs = "riesgos_ts") {
  #' --------------------------------------------------------------------------
  #' Error metrics calculation for fitted models by subregion
  #' Returns the two best predictions based on lowest Score
  #' --------------------------------------------------------------------------
  
  #' Identify prediction columns (ending with _ts), excluding the observed column
  ...
  #' Calculate error metrics for each fitted model
...
resultados_modelos <- rbind(
  calcular_metricas_sub(df_vc1, "VC1", "Riesgo_relativ_contagio"),
  calcular_metricas_sub(df_vc2, "VC2", "Riesgo_relativ_contagio"),
  calcular_metricas_sub(df_vc3, "VC3", "Riesgo_relativ_contagio")
)
...
#' dataset with residuals from the fitted model
df_con_crossbasis_riesgos_vc <- df_con_crossbasis_riesgos_vc %>%
  mutate(
...
#' Extract Pearson residuals by climate subregion

resid_pearson_vc1 <- df_con_crossbasis_riesgos_vc %>%
  filter(Subreg_climat == "VC1") %>%
  pull(resid_pearson_vc)

resid_pearson_vc2 <- df_con_crossbasis_riesgos_vc %>%
  filter(Subreg_climat == "VC2") %>%
  pull(resid_pearson_vc)

resid_pearson_vc3 <- df_con_crossbasis_riesgos_vc %>%
  filter(Subreg_climat == "VC3") %>%
  pull(resid_pearson_vc)

#' Extract Deviance residuals by climate subregion

resid_deviance_vc1 <- df_con_crossbasis_riesgos_vc %>%
  filter(Subreg_climat == "VC1") %>%
  pull(resid_deviance_vc)

resid_deviance_vc2 <- df_con_crossbasis_riesgos_vc %>%
  filter(Subreg_climat == "VC2") %>%
  pull(resid_deviance_vc)

resid_deviance_vc3 <- df_con_crossbasis_riesgos_vc %>%
  filter(Subreg_climat == "VC3") %>%
  pull(resid_deviance_vc)
...
autoplot(df_vc1$riesgos_ts, series = "Observed") +
  autolayer(df_vc1$glmm_ts, series = "GLMM") +
  autolayer(df_vc1$glm_dlnm_ts, series = "DLNM - GLMM") +
  labs(
    x = "Time (weeks)",
...
autoplot(df_vc2$riesgos_ts, series = "Observed") +
  autolayer(df_vc2$glmm_ts, series = "GLMM") +
  autolayer(df_vc2$glm_dlnm_ts, series = "DLNM - GLMM") +
  labs(
    x = "Time (weeks)",
    y = "Hospital discharge risk"
  ) +
...
autoplot(df_vc3$riesgos_ts, series = "Observed") +
  autolayer(df_vc3$glmm_ts, series = "GLMM") +
  autolayer(df_vc3$glm_dlnm_ts, series = "DLNM - GLMM") +
  labs(
    x = "Time (weeks)",
    y = "Hospital discharge risk"
  ) +
...
#' Lagged effects are presented as an example for VC1
## 9.2 Predictions ----
...
save(
  ...
  coef_cond,
  vcov_full,
  file = "random_effects_models/dlnm_models_for_risk_vc.RData"
)

load("random_effects_models/dlnm_models_for_risk_vc.RData")
...
#' Create predictions for temperature with crosspred (DLNM-GLMM, subregion VC1)
...
#' Create predictions for precipitation with crosspred (DLNM-GLMM, subregion VC1)
...
#' Create predictions for relative humidity with crosspred (DLNM-GLMM, subregion VC1)
...
#' Create predictions for AOD with crosspred (DLNM-GLMM, subregion VC1)
...
plot(
  pred_dlnm_glmm_vc1_temp,
  "slices", 
  var = 26, # specific predictor value
  col = 2,
  ylab = "RR"
)
...
#' Lag-response plot for a specific precipitation value (var = 40 mm)
plot(
  pred_dlnm_glmm_vc1_prec,
  "slices",
  var = 40,
...
#' Lag-response plot for another specific precipitation value (var = 5 mm)
plot(
  pred_dlnm_glmm_vc1_prec,
  "slices",
  var = 5,
...
#' Lag-response plot for another specific AOD value (var = 5e-04)
plot(
  pred_dlnm_glmm_vc1_aod,
  "slices",
  var = 5e-04,
...
#' Generate temperature predictions with crosspred (DLNM-GLMM, VC2)
pred_dlnm_glmm_vc2_temp <- crosspred(
  cb_t2,
  coef = coef_t,
  vcov = vcov_t,
  model.link = "log",
  cen = median(filter(data_vc_subreg, Subreg_climat == "VC2")$Temp_media, na.rm = TRUE),
  bylag = 0.1
)

#' Generate precipitation predictions with crosspred (DLNM-GLMM, VC2)
pred_dlnm_glmm_vc2_prec <- crosspred(
  cb_p2,
  coef = coef_p,
  vcov = vcov_p,
  model.link = "log",
  cen = median(filter(data_vc_subreg, Subreg_climat == "VC2")$precip_media, na.rm = TRUE),
  bylag = 0.1
)

#' Generate relative humidity predictions with crosspred (DLNM-GLMM, VC2)
pred_dlnm_glmm_vc2_rh <- crosspred(
  cb_rh2,
  coef = coef_rh,
  vcov = vcov_rh,
  model.link = "log",
  cen = median(filter(data_vc_subreg, Subreg_climat == "VC2")$RH, na.rm = TRUE),
  bylag = 0.1
)

#' Generate AOD predictions with crosspred (DLNM-GLMM, VC2)
pred_dlnm_glmm_vc2_aod <- crosspred(
  cb_aod2,
  coef = coef_aod,
  vcov = vcov_aod,
  model.link = "log",
  cen = median(filter(data_vc_subreg, Subreg_climat == "VC2")$aerosol, na.rm = TRUE),
  bylag = 0.1
)
...
#' Generate temperature predictions with crosspred (DLNM-GLMM, VC3)
pred_dlnm_glmm_vc3_temp <- crosspred(
  cb_t3,
  coef = coef_t,
  vcov = vcov_t,
  model.link = "log",
  cen = median(filter(data_vc_subreg, Subreg_climat == "VC3")$Temp_media, na.rm = TRUE),
  bylag = 0.1
)

#' Generate precipitation predictions with crosspred (DLNM-GLMM, VC3)
pred_dlnm_glmm_vc3_prec <- crosspred(
  cb_p3,
  coef = coef_p,
  vcov = vcov_p,
  model.link = "log",
  cen = median(filter(data_vc_subreg, Subreg_climat == "VC3")$precip_media, na.rm = TRUE),
  bylag = 0.1
)

#' Generate relative humidity predictions with crosspred (DLNM-GLMM, VC3)
pred_dlnm_glmm_vc3_rh <- crosspred(
  cb_rh3,
  coef = coef_rh,
  vcov = vcov_rh,
  model.link = "log",
  cen = median(filter(data_vc_subreg, Subreg_climat == "VC3")$RH, na.rm = TRUE),
  bylag = 0.1
)

#' Generate AOD predictions with crosspred (DLNM-GLMM, VC3)
pred_dlnm_glmm_vc3_aod <- crosspred(
  cb_aod3,
  coef = coef_aod,
  vcov = vcov_aod,
  model.link = "log",
  cen = median(filter(data_vc_subreg, Subreg_climat == "VC3")$aerosol, na.rm = TRUE),
  bylag = 0.1
)
# 10. Forecasts ----
...
# Set common levels for year in both train and test sets
...
# Prediction and forecasting using DLNM-GLMM Normal
df_train <- df_train %>%
...
# Prediction and forecasting using GLMM
df_train <- df_train %>%
